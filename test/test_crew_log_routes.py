...
class TestTheGrantIsRecheckedAfterTheRead:
    ...
    def test_no_test_asserts_a_caller_class_recheck_because_it_would_overclaim(self):
        """The re-check tests the TARGET's class and deliberately not the caller's.

        A caller's class says where its turn LANDS, and that is decided when the turn
        is emitted -- after the route has answered. A caller that acquires a channel
        link one instant after the payload returns, or that still holds the payload in
        context on its next turn, publishes the same content with nothing to stop it,
        and no refusal can claw back a read already granted. So a caller-class re-test
        would shrink one race window by the duration of a read while the adjacent
        instant stayed open, and would read in the source as a guarantee this route
        cannot make. The caller's class is tested at the GATE, where it decides whether
        the read happens at all, and ``TestTheCallerClassesAreTestedAtTheGate`` covers
        that. This asserts the absence so the removal is a decision on the record
        rather than a line someone dropped.
        """
        import inspect

        source = inspect.getsource(routes._stale_grant_refusal)
        assert "_caller_class_refusal" not in source, (
            "the re-check calls the caller-class test again; that cannot buy an "
            "invariant and states one it does not hold"
        )
    ...
    def test_every_offloading_route_rechecks_before_it_answers(self):
        ...
        an agent-door handler that suspends to build ONE NAMED UNIT's payload must pass
        through ``_stale_grant_refusal`` before returning it, because that payload is
        delivered under a verdict about that unit's class and the class can move while
        the entries are read.

        The listing is the one offloading handler that does not, and it is named here
        rather than skipped by a condition that could quietly widen: its scope is a
        filter rather than a verdict on rows, so the only thing that could move under
        it is the CALLER's class, and a caller-class re-test cannot buy an invariant --
        where a turn lands is decided after the route returns.
        """
        import ast
        import inspect
        ...
        for node in ast.walk(tree):
            ...
            if "unit" not in names:
                listing_routes.append(node.name)
                continue
            unit_routes.append(node.name)
            assert "_stale_grant_refusal" in called, (
                f"{node.name} suspends after authorizing one unit's read but never "
                "re-checks that unit's class; it can have moved while the read ran"
            )
        # Controls: an empty set would satisfy the loop above silently, and the
        # exemption is asserted by NAME so a second exempt route cannot appear quietly.
        assert len(unit_routes) == 2, f"expected two unit routes, found {unit_routes}"
        assert listing_routes == ["api_crew_log_sessions"], listing_routes
