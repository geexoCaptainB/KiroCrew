...
def test_session_manager_reclaim_is_explicit_not_constructor_io():
    from kiro_crew.config import KiroCrewConfig
    from kiro_crew.session import SessionManager

    with patch(
        "kiro_crew.session.SessionMap.reclaim_adopted_transcripts", return_value=2
    ) as reclaim:
        manager = SessionManager(KiroCrewConfig())
        reclaim.assert_not_called()
        assert manager.reclaim_adopted_transcripts() == 2
        reclaim.assert_called_once_with()


def test_session_manager_constructors_reclaim_at_safe_lifecycle_point():
    cases = {
        "cli.py": (
            "sessions = SessionManager(",
            "sessions.reclaim_adopted_transcripts()",
            "sessions=sessions",
        ),
        "cli_server.py": (
            "sessions = SessionManager(",
    ...
    constructors = {
        path.relative_to(src_root).as_posix(): text.count("SessionManager(")
        for path in src_root.rglob("*.py")
        if (text := path.read_text(encoding="utf-8")).count("SessionManager(")
    }
    assert constructors == {
        **{name: 1 for name in cases},
        "slack/gateway.py": 1,
    }, constructors
    for name, (construct, reclaim, first_use) in cases.items():
        text = (src_root / name).read_text(encoding="utf-8")
        assert text.index(construct) < text.index(reclaim) < text.index(first_use), name


def test_gateway_reclaims_only_after_ready_and_the_initial_prune():
    """Gateway migration is maintenance: serving and prune fencing come first."""
    text = (REPO_ROOT / "src" / "kiro_crew" / "slack" / "gateway.py").read_text(encoding="utf-8")
    init_services = text.split("async def _init_services", 1)[1].split(
        "async def _open_dm_with_retry", 1
    )[0]
    assert "reclaim_adopted_transcripts" not in init_services

    ready_at = text.index('print(f"KIROCREW_READY:')
    start_bg_at = text.index("async def _start_bg_session()", ready_at)
    prune_at = text.index("await self.sessions.start_pool(blocking=False)", start_bg_at)
    reclaim_at = text.index(
        "await asyncio.to_thread(self.sessions.reclaim_adopted_transcripts)", prune_at
    )
    assert ready_at < start_bg_at < prune_at < reclaim_at
...
class TestAdoptedHomeKeepsSessionMap:
    ...
    def test_prune_before_reclaim_keeps_a_host_side_mapping(self, monkeypatch, tmp_path):
        """Post-READY migration may start after pool startup's initial prune."""
        sm_mod, scratch, host_sessions = self._relocated_install(monkeypatch, tmp_path)
        (host_sessions / "sid-a.json").write_text("{}", encoding="utf-8")
        sm = sm_mod.SessionMap()
        sm.set("dashboard:one", "sid-a")
        ...
        assert sm.reclaim_adopted_transcripts() == 1
        assert (isolated_kiro_home(scratch) / "sessions" / "cli" / "sid-a.json").is_file()

    def test_failed_reclaim_survives_prune_and_retries(self, monkeypatch, tmp_path):
        """A mundane move failure retains both the source and its resumable map row."""
        sm_mod, scratch, host_sessions = self._relocated_install(monkeypatch, tmp_path)
        (host_sessions / "sid-a.json").write_text("{}", encoding="utf-8")
        (host_sessions / "sid-a.jsonl").write_text('{"role": "user"}\n', encoding="utf-8")
        sm = sm_mod.SessionMap()
        sm.set("dashboard:one", "sid-a")
        real_move = sm_mod._move_transcript_no_follow
        attempts: list[Path] = []
        ...
        monkeypatch.setattr(sm_mod, "_move_transcript_no_follow", _fail_move)
        assert sm.reclaim_adopted_transcripts() == 0
        assert attempts == [
            host_sessions / "sid-a.json",
        ...
        monkeypatch.setattr(sm_mod, "_move_transcript_no_follow", real_move)
        assert sm.reclaim_adopted_transcripts() == 1
        assert (isolated_kiro_home(scratch) / "sessions" / "cli" / "sid-a.json").is_file()
        assert sm.prune() == 0
        assert sm.get("dashboard:one") == "sid-a"

    def test_get_before_reclaim_keeps_the_mapping_and_the_sid(self, monkeypatch, tmp_path):
        """A request-driven get() in the post-READY window must not repair the row away."""
        sm_mod, scratch, host_sessions = self._relocated_install(monkeypatch, tmp_path)
        (host_sessions / "sid-a.json").write_text("{}", encoding="utf-8")
        (host_sessions / "sid-a.jsonl").write_text('{"role": "user"}\n', encoding="utf-8")
        sm = sm_mod.SessionMap()
    ...
    def test_a_half_finished_move_is_completed_on_the_next_start(self, monkeypatch, tmp_path):
        """The transcript moves before the journal, and each file is judged on its
        own, so a move that dies between the two is not read as done: the prune
        that follows in the same startup keeps the entry (it keys on the
        transcript, already across), and the next start moves what is still on the
        host side. Nothing is stranded and the mapping survives."""
        ...
        monkeypatch.setattr(sm_mod, "_move_transcript_no_follow", _die_on_journal)
        assert sm.reclaim_adopted_transcripts() == 1
        assert (adopted / "sid-a.json").is_file() and not (adopted / "sid-a.jsonl").exists()
        assert (host_sessions / "sid-a.jsonl").is_file()  # still on the host side
        # The same startup's prune must not drop the half-moved entry (prune keys
        # on the transcript, which is already across). ``get`` is not consulted
        # here: it also wants the journal, so it would read this one session as
        # stale until the next start finishes the move -- a bounded window, not
        # a lost mapping.
        assert sm.prune() == 0 and sm.has_hint("dashboard:one")
    ...
    def test_a_dangling_link_at_the_destination_is_not_followed(self, monkeypatch, tmp_path):
        ...
        assert sm.reclaim_adopted_transcripts() == 1  # the journal moved
        assert not outside.exists()
        assert (adopted / "sid-a.json").is_symlink() and not (adopted / "sid-a.json").exists()
        assert (host_sessions / "sid-a.json").read_text(encoding="utf-8") == '{"secret": 1}'
        assert (adopted / "sid-a.jsonl").is_file() and not (host_sessions / "sid-a.jsonl").exists()
    ...
    def test_a_symlink_under_a_mapped_sid_on_the_host_side_is_not_read_through(
        ...
        assert [p.name for p in adopted.iterdir()] == ["sid-a.jsonl"]
    ...
    def test_destination_directory_fsync_failure_keeps_source_and_removes_destination(
        ...
        with pytest.raises(OSError, match="destination-directory fsync"):
            sm_mod._move_transcript_no_follow(src, dst)
    ...
    def test_unsupported_directory_fsync_does_not_abort_the_migration(self, monkeypatch, tmp_path):
        ...
        rather than failing every transcript and letting the prune that follows
        drop the mappings."""
        from kiro_crew import session_map as sm_mod
        ...
        assert dst.read_text(encoding="utf-8") == '{"session": 1}'
