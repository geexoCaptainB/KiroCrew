...
def test_the_unit_order_write_never_follows_a_planted_link(tmp_path):
    ...
    # A link at the DIRECTORY: refused, the record still lands, the target is untouched.
    victim_dir = tmp_path / "operator-dir"
    victim_dir.mkdir()
    crews = cs.crews_dir(OWNER, REPO, tmp_path)
    keep = crews.with_name(crews.name + ".keep")
    crews.rename(keep)
    os.symlink(victim_dir, crews, target_is_directory=True)
    try:
        cs._fold_cache.clear()
...
def test_a_legacy_row_that_cannot_fit_leaves_the_carry_unfinished(tmp_path, monkeypatch):
    """A valid row the log will not take is not carried, and the carry is NOT marked
    finished -- the finished marker would discard stored state for good. The rows
    that fit land; the next write tries the rest again."""
    crew, sid = _live_crew(tmp_path)
    cid = crew["id"]
    _legacy_item(tmp_path, cid, 2251)
...
def test_a_malformed_pre_projection_file_is_skipped_not_fatal(tmp_path):
    crew, sid = _live_crew(tmp_path)
    cid = crew["id"]
