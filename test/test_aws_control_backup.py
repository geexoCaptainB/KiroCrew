...
class TestSessionsArchiveLayerBGate:
    ...
    def test_an_agent_writable_config_key_cannot_grant_layer_b(self, tmp_path, monkeypatch):
        ...
        recalled. Every spelling an earlier draft of this gate read is offered
        here at once, so moving the read back into ``config.json`` under any of
    ...
    def test_the_upload_holds_the_setter_lock_across_the_permission_read(
        ...
        Probed from inside ``put_file`` by asking, without blocking, whether the
        setter's lock is available. Non-blocking on purpose: a blocking
        acquisition would deadlock against the very lock this asserts is held.
        """
        self._skip_without_pinning()
        self._both_halves(tmp_path, monkeypatch)
        self._store(tmp_path, True)
        ...
        def _lock_is_free_to_another_thread() -> bool:
            # Another thread is what a concurrent revocation request is. The
            # setter's first gate is `_run_lock`, so its availability answers
            # whether this call sits inside the setter's critical section.
            answer: dict[str, bool] = {}

            def probe() -> None:
                acquired = backup._run_lock.acquire(blocking=False)
                answer["free"] = acquired
                if acquired:
        ...
        def fake_put(
            ...
            seen[which] = _lock_is_free_to_another_thread()
    ...
    def test_a_refused_authorization_releases_the_lock_it_took(self, tmp_path, monkeypatch):
        ...
        `_authorize_upload` raises from within the critical section now. A lock
        leaked on that path would wedge every later backup and every revocation,
        which is worse than the staleness the wider scope exists to prevent.
        """
        self._skip_without_pinning()
        self._both_halves(tmp_path, monkeypatch)
        self._store(tmp_path, True)
        ...
        # Nothing shipped, and the lock is free again. Probed without blocking for
        # the same reason as the sibling release test.
        put.assert_not_called()
        released: dict[str, bool] = {}

        def probe() -> None:
            acquired = backup._run_lock.acquire(blocking=False)
            released["free"] = acquired
            if acquired:
                backup._run_lock.release()
    ...
    def test_a_refused_upload_releases_the_lock_it_took(self, tmp_path, monkeypatch):
        ...
        The refusal raises from inside the critical section. A lock leaked there
        would be worse than the race it closes: every later backup and every
        revocation takes the same lock, so they would all wait forever on a
        permission decision that already finished. Release comes from the
        ...
        # Probed from another thread, without blocking. Asserting by acquiring the
        # lock here would HANG on a leak rather than fail: the file lock is
        # per-descriptor so a second exclusive take blocks, and `_run_lock` is an
        # RLock that this thread could re-enter regardless. A non-blocking probe
        # from a thread that holds nothing answers cleanly either way.
        released: dict[str, bool] = {}
