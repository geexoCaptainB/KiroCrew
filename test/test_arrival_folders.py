...
async def test_a_failed_import_takes_back_the_folder_it_created():
    ...
    await af.discard_arrival_folders(state, filing.created_ids)
...
@pytest.mark.asyncio
async def test_the_rollback_spares_a_row_a_later_arrival_adopted():
    ...
    ``from mac``, the second adopted it and then archived, and the first one's
    failure would delete a placement that archived session still points at.

    The marker written at adoption time is what closes it, and it is per ROW: a
    row nobody adopted is still reclaimed in the very same rollback, which is the
    leak this rollback exists to close.

    Mutation-checked, and one test kills both variants. Not writing the marker on
    adoption deletes ``from mac``; inverting the test to skip an UNMARKED row
    spares ``from pc`` and deletes ``from mac``, so either way this fails.
    """
    state = _store()
    ...
    assert rows[first.folder_id].get(af.ARRIVAL_ADOPTED_KEY), "the premise: adoption was recorded"
    ...
    state._slots = {}
    await af.discard_arrival_folders(state, (*first.created_ids, other.folder_id))
