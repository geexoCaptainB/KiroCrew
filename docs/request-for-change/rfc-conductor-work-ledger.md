...
The reason this is not simply "let the worker call `session_send`" is in the code that withholds it, quoted in full in §2. `session_send` is not merely ungranted to a worker: it lives on the opt-in `kirocrew-dashboard` server, which a worker spec never mounts, so the tool never exists in a worker's surface. A worker's only ways to reach a human are `send_message` and `send_notification`, which address a *person*, not a session's turn queue. The ledger is a worker's only channel to its conductor.
...
Returns the whole board: the conductor record, every item with all fields, each item's derived `orphaned`, `stale`, `acceptance_concrete` and `unread_inbox` flags, the newest events per item — including every `request`, `channel_open`, `channel_close` and `message` — and a ready-to-pipe `accept_batch` holding the `{"items": [...]}` document `accept_eval.py` expects, built from `acceptance` only and never from the worker's claimed `pr`. An item whose bar is not concrete yet is left out of the batch, because `accept_eval.py` can only answer `error` to such a spec and a conductor reading a column of verdicts would take that for a failure of the work. Not concrete means a `kind` that evaluator does not dispatch on, a placeholder string (`TBD`, blank) in a field it READS for that kind, or one of those fields mistyped — a `pr_checks` `pr` that is not a positive integer, a `file` `path` that is not a string, a `file` `exists` that is not a bool. Those are exactly that script's error-only guards, mirrored across the process boundary and pinned against its real behaviour by a test, with the kind vocabulary derived from its own dispatch so a new kind there cannot silently drop every item using it. The judgement is field by field over what it consumes and never a walk of the stored object: an acceptance is stored verbatim and may carry metadata the evaluator never looks at, which cannot change a verdict and so must not cost an item its place — and a read-path recursion over caller-supplied nesting is a depth an untrusted record could exhaust, failing every later batch read of that slot. A `cmd` bar deliberately stays in the batch too, because its `refused` verdict is a message the conductor must receive rather than an item that quietly disappears. `acceptance_concrete` on the item row is what says so. Each batch entry carries its item's `status` as well, so the conductor's own "`done` only" filter needs no second lookup — the read does not apply that filter, which stays the conductor's judgement. Phase 5 adds a `channels` list of the open pairs with their expiry and remaining budget.
...
**A worker edits the ledger files directly.** The two "unrepresentable" rows above hold only for the tools: a worker is a full-capability agent with `fs_write`, so a prompt-injected one could bypass `work_report` and open `items/it_*.json` itself — forging `state: accepted`, gutting `acceptance` before the conductor's evaluator runs, or reading a sibling item past `read_work_brief`'s scoping — and the conductor would read the forgery as its own writing. Prevented by the file fence, on two planes. The file-tool plane: `work-ledger` is a directory leaf in `_CREW_SECRET_LEAVES` ([`src/kiro_crew/security/paths.py`](../../src/kiro_crew/security/paths.py)), which projects it into `_SENSITIVE_HOME_DIRS` under both crew data-home prefixes and re-anchors it under a `KIROCREW_HOME` override, so `is_sensitive_path` is true for every path under `<data_home>/work-ledger/` and `hooks.on_tool_call` refuses the agent's read and edit tools there — read AND write, because reading a sibling's record is the scoping bypass and writing one is the forgery. The shell plane: the same leaf is in `sandbox._CREW_HIDDEN_LEAVES`, so `echo >` and `python -c` inside the agent's sandbox find no such directory; command text is deliberately not matched (see `is_sensitive_bash_command`), because a text matcher denies ordinary commands whenever a fenced spelling appears as data. The store's own `atomic_write` runs in the gateway process and opens the paths directly, not through the gate, so the sanctioned writers are unaffected. A directory leaf rather than a file leaf for the reason the sibling `ledger` entry is one: the store publishes through a `mkstemp` sibling, and fencing final names only leaves the same bytes writable under a temp name.
...
- `kirocrew-worker`: `"@kirocrew-work"` added to the default agent's own resolved surface, with `_WORKER_WORK_GRANTS` auto-approving `work_brief` and `work_report`. A worker that must ask permission to say it is blocked will not say it. The keys in `_WORKER_MIRRORED_SHAPES` — `tools`, `allowedTools`, `excludedTools`, `mcpServers`, `model` — are mirrored from `kirocrew.json` **as it stands on disk**, not from the template that file was assembled from: a user's own servers arrive by app registration, `mcp.json` merge and dashboard mount, and their model by `config.json`, none of which `build_agent_config` sees. So a server or grant added to the default agent reaches the worker on the next refresh, and one the governance ceiling withholds there stays withheld here. The previous worker file contributes exactly one field: a `model` frozen by an explicit pick (`model_managed` False), which outranks the mirror -- and it is TYPE-CHECKED before it is carried across, on the same grounds the mirror loop checks the default's own keys. That value arrives from the dashboard's model PATCH and from the file itself, neither of which guarantees a string, and kiro-cli validates the spec strictly, so copying a number, a list or a null through would write a worker spec the agent cannot load at all -- a cosmetic bad pin turned into a worker that will not start. Anything that is not a non-empty `str` (a blank one names no model) loses to the mirrored default, which is the recoverable direction, and is reported by KEY and offending TYPE rather than by value, since the field can hold whatever a PATCH put there. Nothing else crosses — the worker file is derived, so an entry in it is either a copy of the default's or the user's own and nothing on disk says which, and an add-only merge therefore resurrects a server the default has since dropped (register an app, deregister it, and its tools stay callable on the worker) while re-admitting an `autoApprove` no ceiling has seen. The final `mcpServers` map goes through `_strip_ungoverned_auto_approve` for that second reason, and the read and write sit under `agents_spec_lock` together like every other template-spec read-modify-writer. The guarantee is stated at the SPAWN, not at the writers: **a worker never starts on a mirror older than the default spec.** `require_fresh_derived_spec` runs EXACTLY ONCE on every spawn path, re-derives a stale mirror, and REFUSES the spawn (`DerivedSpecStale`) when it cannot -- the same reasoning `require_fork_governance` applies to an unprojected fork, and for the same hazard: a mirror predating a trust revocation still mounts and auto-approves a server the default no longer has. Staleness is a CONTENT fingerprint of the mirrored keys (recorded per agent in the `agent_state` sidecar, since kiro-cli drops a spec carrying an unknown key). Every pairing of a fingerprint with a file identity comes from ONE observation of the default spec, bracketed stat-read-stat, and never from two: an identity taken by a fresh stat beside a fingerprint read back out of the sidecar is a TORN pair -- the new generation's identity carrying the old generation's hash -- and a check that matched the identity would then accept a mirror built from different content. The gate therefore hashes the bytes it read (`_spec_fingerprint`) rather than reading a recorded value, re-stats afterwards to prove the file never moved, and re-runs the whole verification when it did; bounded at `_DEFAULT_SPEC_OBSERVATION_ATTEMPTS` attempts, because the loop's exit condition is another process leaving the file alone and a spawn that never returns is worse than one that refuses. The same rule binds the DERIVE end: the fingerprint recorded at derive time is of the bytes that derivation mirrored, and the identity is recorded only when a re-stat proves the file held still -- otherwise it is CLEARED, which costs one re-derive instead of licensing a false match. The sidecar's identity is thus only ever a fast path that skips a re-derive, and that fast path confirms identity equality both before and after reading it, since the read is itself a window. The fast path that avoids hashing is an IDENTITY test on the default spec alone -- its current `(st_mtime_ns, st_size, st_ino)` against the tuple recorded at derive time -- and never an ordering test between the two specs' timestamps: a restored backup and a clock that steps backwards leave a default spec that is *older* than the mirror while holding different content, which is exactly the case "the mirror is newer, so it is fresh" gets wrong. Any difference falls through to the hash. The refusal is raised OUTSIDE each spawn path's best-effort materialization handler and converted to that path's abort type, so a broad `except` cannot log it and spawn anyway. The SPAWN is not the only load: a `session/set_mode` naming an agent is a kiro-cli load of that agent's spec from disk, and it boots that agent's MCP servers, so it carries the same bracket. Neither other gate reaches it -- the spawn gate is keyed to the runtime's OWN agent, so a shared runtime spawned as one agent and switched to `kirocrew-worker` on that line passes no gate, and this host builds no in-process tool surface, so the projection's gate does not run either. Both activation paths (`create_session` and the resume in `load_session`) therefore re-verify a snapshot AFTER the send returns, tearing the session down on a change exactly as the `initialize` bracket does. WHICH snapshot is the whole question, and it is decided by where the spec is CONSUMED -- exactly one snapshot per consumed load. On a host that reads from disk at `set_mode` (kiro), the activation IS the load, so it gates there too: refusing without sending, and terminating the session `session/new` already created rather than leaving it half-registered. On a wire-registered host (KAS) the spec was consumed at `session/new`, inside the `customAgents` payload the projection built under its own gate, and `set_mode` only ACTIVATES what is already registered without re-reading anything -- so the payload's snapshot travels out of `session_extras` on `SessionExtras.derived_spec_snapshot` and the post-check compares against THAT. A fresh read at activation would judge the FILE while the host holds the PAYLOAD: a revocation landing between the build and the activation passes that read while the registered definition still carries the grants it removed, which is the same class of defect as gating twice for one spawn. There is deliberately no gate call on the wire path. The array-backed hosts (Claude, Codex, OpenCode) are the third consumption shape: they read no spec of their own, the `session/new` `mcpServers` array IS the spec they consume, and the same array is re-sent on `session/load`. So the snapshot the projection verified travels with the array -- `SessionMcpProjection.derived_spec_snapshot` out of the one reader, `SessionProjection.derived_spec_snapshot` through each mirror, cached on the client beside the array itself -- and the client re-verifies it ONCE, after the session exists by whichever route created it and before anything runs on it. That is the full set: eight consumption points across three shapes (disk read at spawn, disk read at `set_mode`, wire payload at `session/new`), each bracketed by exactly one snapshot and re-verified where the host provably consumed it; the `set_mode` bracket is one shared helper (`_activate_mode_bracketed`) for both session-start paths, so a protocol edit lands once. Fail-closed is right for a REVOCATION; it is the wrong surface for a benign write -- the dashboard's MCP sync, an app registration -- that happens to land inside a spawn's window, so the runtime's spawn path retries exactly once on the bracket's own refusal: the second attempt re-runs the gate, which re-derives the mirror from the default as it now stands, and a default that is still moving across two spawns is refused as before. The client already had this retry. An AST test enumerates every `set_mode` send site with an agent override and asserts the gate precedes it and the post-load check follows it, by statement position rather than by mention. On the harness-driven spawn the gate belongs to `AcpRuntime` and to nothing else -- one call, taken AFTER the host's own pre-spawn work, as the last verification before the process is created. It is the one host-level gate the runtime owns rather than the harness, for two reasons: it is the same check for every host, and it opens a bracket the runtime's own handshake closes, so both ends have to belong to the object that drives the handshake. A SECOND call inside a harness is a defect rather than redundancy, because the gate may re-derive: the subprocess then loads the NEWER spec while the post-handshake check compares against the older snapshot and kills a session that was never stale. Taking it last also puts the host's materialization self-heal first, so a missing default spec is repaired on the path that can repair it instead of refused. An AST test counts the calls (exactly one in the runtime's plan resolver, zero in every harness spawn seam) and asserts the capture follows the harness call; a companion test proves the seam has exactly one caller in the product, which is what licenses the kiro harness materializing without gating. kiro-cli reads the worker spec ITSELF, in another process, milliseconds after the gate passed, so the pre-check alone leaves a verified-then-changed window. Holding a writer lock across a subprocess's read is not available, so the window is closed by DETECTION: the gate returns a `DerivedSpecSnapshot`, and each of the two subprocess spawners re-verifies it after the `initialize` response -- the earliest point kiro-cli has provably consumed its spec -- and before it creates a session, so a session is never built on a spec nobody verified. A difference, or a re-check that cannot be performed, ends the session. In `AcpRuntime` the check sits inside the runtime's own `except BaseException` guard, which kills the process, reaps its PID-file entries and protected-PID shield, and re-raises. In `AcpClient` it is converted to `AcpError`, which is the type `ensure_ready` handles and therefore the one that ends the child and drops the half-registered session state (`_cleanup_failed_live_spawn` kills the process and releases the workspace binding, `_reset_state` closes the pipes and clears the session id and per-spawn spec caches); the single retry that handler allows respawns, so it goes back through the pre-spawn gate rather than continuing on the child that may have read an unverified spec. A raw `DerivedSpecStale` there would match none of those handlers and leave that child alive, which is why the conversion is load-bearing on both gates of that path. Closing the bracket at SPAWN time is not an option on either path: the child has not read its spec yet, so the capture is the only half a spawn point can hold. The spawner set is derived by test from "runs the gate and sends `initialize`" rather than listed, so a third spawner cannot ship the capture half alone. A write landing before that point is caught, and one landing after cannot change what the subprocess already read. The two IN-PROCESS sites (`session_mcp.agent_spec_snapshot`, `KasHarness.session_extras`) carry no such window by construction: for a derived agent each performs ZERO reads of the spec and projects `snapshot.spec` -- the parsed spec the gate itself read and verified, carried on the `DerivedSpecSnapshot`. One read placed carefully AFTER the gate returns is still a second observation of a file the gate had already finished with, however tight the sequence looks, and a revocation landing between the two reaches the session as its whole tool surface as though it had been checked; no lock closes that, because both halves are this process's own reads. So the gate reads the mirror inside its own stat-read-stat window -- bracketed on the mirror's identity as well as the default's, so the bytes handed out belong to the same instant as the verification that vouches for them -- and the consumers are handed the object. An unreadable mirror there is a `DerivedSpecStale` refusal rather than a projection of nothing. Every other agent mirrors nothing, has no snapshot, and is read at the site as before. On the KAS side that is a property of the projection's SIGNATURE rather than of its callers -- `build_kas_custom_agents` takes the verified spec as a required positional argument and performs no read of its own, because a builder that read the file itself would be a second read milliseconds after the gate, and a revocation landing in between would become the session's whole tool surface as though it had been checked. No lock closes that window, since both halves are this process's own reads, so the second read is removed rather than re-checked; the parameter is deliberately not defaulted, because a default that fell back to reading would restore the hole for any caller that forgot to pass one. Both sites are pinned by tests so a refactor that re-read the file would fail. A project-local SHADOW is refused outright: kiro-cli resolves `--agent` against `<cwd>/.kiro/agents/*.json` as well as the user level, matching on the spec's declared `name` rather than its filename, so a checkout could supply its own `kirocrew-worker` with any `autoApprove` it liked and a verified global derivation would prove nothing about the spec the session gets. There is no override knob and no repair: the file belongs to the checkout, so rewriting it would have Crew editing tracked repository content, and honouring it would let a cloned repo choose a dispatched worker's grants. Only DERIVED agents are refused this way; the general case -- any managed agent shadowed by a checkout, `kirocrew.json` included -- is pre-existing and tracked in #10553. The gate returns only on POSITIVELY established freshness -- a matching identity tuple, a matching fingerprint, or a re-derive that succeeded -- and its body carries no `return` statement at all, so falling off the end is unreachable without one of those. Every other outcome raises with the concrete reason: a missing default spec, one that exists but cannot be read (oversized, not JSON, refused at the read gate), or a failed re-derive. An unreadable default is NOT answered as "not fresh" either, because a re-derive that cannot read it silently produces a template-based worker carrying none of the user's servers or revocations. An unwritable SIDECAR is the one unverifiable input that is recoverable: it is how freshness is proven cheaply next time, not what makes the spec correct, so a successful re-derive stands on its own. Hanging a re-derive off each WRITER was tried and removed: `kirocrew.json` has six write sites across three modules under two different file locks -- the rebuild, three app-registration paths, the dashboard MCP sync and the agent-config PUT -- so that shape leaks one hole per writer nobody named, and a post-write hook can still lose the race to a spawn. `rebuild_agent_config` keeps deriving at boot. One ceiling pass covers every source, so a governed host gets a prompt rather than a bypass — including for a grant read back out of the file a looser boot wrote. The mirror also carries no managed `opt_in` server other than `kirocrew-work` (`_worker_unassignable_servers`), which is what keeps the structural-absence claim above TRUE rather than conditional: an opt-in set is assigned per agent, so `kirocrew-dashboard` mounted on the operator's own default agent must not put `session_send` in a worker's surface. `_WORKER_EXCLUDED_GRANTS` is the other SUBTRACTION from the mirror: `cron_add`, `cron_update` and `cron_secret_request` are never auto-approved on a worker, because a recurring job outlives the item, the session and the dispatch. A whole-server `@kirocrew-cron` grant on the default is replaced by the shipped template's per-tool cron grants rather than carried across, since a whole-server grant covers the excluded verb too; the tool stays mounted, so an item that genuinely needs a schedule reaches the approval gate instead of failing silently. The predicate (`_grant_reaches_excluded`) classifies EVERY entry, not only the `@`-prefixed ones: `_canonical_grant_pattern` answers `None` for an entry that is not an MCP server ref, and reading that as "reaches nothing" was a fail-OPEN hole, because a bare `"*"` is kiro-cli's spelling for *every tool* and so auto-approves `cron_add` while skipping the check entirely. An entry with no `@` is a glob over the WHOLE namespace -- it reaches further than any server-scoped ref, not less far -- so it is matched against each excluded ref under both spellings the namespace offers, the full `@server/verb` and the bare verb, and either hit counts. Such an entry can only be DROPPED, never narrowed: there is no spelling for "everything except cron_add". The predicate has a single exit taken after classifying, on the same zero-shortcut discipline the freshness gate carries, because every earlier version of this subtraction grew a shortcut for a shape it did not want to think about and each of those was a grant reaching an excluded verb unexamined. The spec is therefore `build_agent_config()`-shaped only on a fresh install, and otherwise `default + @kirocrew-work − cron scheduling`.
...
**What a worker structurally does not have.** `session_send` lives on `kirocrew-dashboard`, which is `opt_in` in `_MANAGED_MCP_SERVERS` and is hand-mounted only by the specs that need it — the conductors and Crew Mode members. A worker spec is not one of them, and the mirror does not make it one: `_worker_unassignable_servers` keeps every managed `opt_in` server except `kirocrew-work` out of the derivation, precisely so this paragraph stays true when an operator mounts the set on their own default agent. So a worker does not have `session_send` withheld by an allowlist it might later be added to; the server carrying it is never in a worker's spec at all. What the worker does have, from `kirocrew-core`, is `send_message` and `send_notification`, and both address a *person* rather than a session's turn queue. The ledger is therefore a worker's only channel to its conductor, and Phase 5's `work_message` its only channel to a peer.
...
The conductor picks the child's agent per item. **As implemented**, this table is in `_CONDUCTOR_SYSTEM_PROMPT` and in `goal-conductor/SKILL.md`, and the middle row names `kirocrew-conductor` itself — a decomposable item's child must be an agent that has the ledger tools, which after the swap is that spec:
...
The swap is what put it in `goal-conductor/SKILL.md`; the deprecated `goal-ledger-conductor` skill is a pointer to that one and carries no table of its own.
...
| 2.5 — isolation | **done**, then superseded by the swap | [#9277](https://github.com/kirodotdev/KiroCrew/pull/9277) |
| 2.6 — the swap: `kirocrew-conductor` becomes the ledger conductor, old name kept as a deprecated alias | **done** | see §Rollout note — swap done |
...
on a plain timer today: it polls with `work_ledger_read` on its nudge interval and pays
one turn per quiet tick. Phase 3 is a cost optimisation on that loop, not the thing that
makes it work — which is what the swap in §Rollout note — swap done settled, by taking
the per-quiet-tick turn as the accepted cost rather than as a blocker.
...
- `<data_home>/work-ledger/**` is on the sensitive-path floor and the sandbox mask (§Security considerations, "A worker edits the ledger files directly"), asserted two ways: by spelling under both crew data-home prefixes, and end to end through `hooks.on_tool_call` against the real item, binding and conductor files of a dispatched worker under a `KIROCREW_HOME` override — an edit and a read are both refused, and the worker's route and the conductor's store action still write the very file the tool was refused. Deferred from the Phase 1 design review as [#9083](https://github.com/kirodotdev/KiroCrew/issues/9083); the fence itself shipped in #9152.
...
Exit criteria, all met at the time — the first and third are superseded by the swap
(§Rollout note — swap done), and only the second still holds as written:
- `kirocrew-conductor` and `kirocrew-pipeline-conductor` emit no `kirocrew-work` entry and no `@kirocrew-work` reference, asserted on the output of both installers. **Superseded:** `kirocrew-conductor` mounts it now; the pipeline conductor still does not.
- `kirocrew-ledger-conductor` auto-approves exactly `work_ledger_read`, `work_ledger_record` and `work_brief`, and not `work_report`. Still true, and now true of `kirocrew-conductor` by the same code path.
- `goal-conductor/SKILL.md` and `ledger_entry.py` are untouched, so an un-migrated conductor mid-goal sees no change. **Superseded:** the SKILL.md carries the ledger procedure; `ledger_entry.py` is still shipped and its deletion is still Phase 4's.
