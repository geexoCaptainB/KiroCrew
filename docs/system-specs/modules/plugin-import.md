...
an npm scope specifier (`@scope/pkg`) and a URL (`git+https://...`). A bare command
...
newline could open a line that reads as the tool's own output.
...
`output_within_source`, `output_is_a_link`, `staging_unwritable`,
`output_publish_failed`, `emitted_manifest_invalid`.
