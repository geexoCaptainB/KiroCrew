...
occupies `slot.task`. Executing, cancelling, and paused-gate conflicts all return
`409` with `code: slot_busy`; the paused-gate message remains distinct and tells
the client to continue from the dashboard with Go, because the OpenAI-compatible
endpoint cannot submit that action.
