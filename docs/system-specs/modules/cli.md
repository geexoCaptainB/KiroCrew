...
19. **Hook auto-approve platform scope**: reports whether a name-based auto-approve can be satisfied on this platform, read off the same `platform_compat.IS_WINDOWS` branch `name_grant.name_grant_refusal` takes, so the row cannot describe a posture the check does not hold. On Windows it prints `⏹ declined on Windows (windows_lookup_not_modelled)` and states that this is the platform's scope rather than the user's configuration — the only other trace of it is a decline line in `gateway.log` per invocation, and what a user would have to read to tell the two apart is the source of `name_grant`. Never an issue and never part of the exit code: the fail-closed is the intended posture, since the POSIX tokenizer does not preserve a backslash path and `cmd.exe` searches the command's own directory before `PATH`, so resolving names loosely there is the planted-shim attack the refusal exists to stop. The same platform-scope classification bounds the log on the three surfaces that consult it: `name_grant.should_log_decline` gates the human-facing warning to once per session, for a code that describes the platform rather than the command, at the agent hook webhook, the dashboard chat runner and `llm_helpers`. The Slack gateway and handler, the messaging driver, `channel`, the subagent runner and the task runner call `name_grant.log_decline` without consulting that gate, so those warn on every decline. `name_grant.log_decline` itself writes the SEL audit row for every decline on every surface, because declining is a security decision and only the human-facing line is ever deduplicated.
20. **Data Home — kiro home** (`_doctor_kiro_home`, inside the Data Home section): under the default data home the kiro-cli home is the machine-wide `~/.kiro` and the line is omitted. Under a non-default `KIROCREW_HOME` the line is always printed: `kiro home: <path> (isolated|KIRO_HOME override; kiro-cli reads agents/sessions here)`, or — under the `KIRO_HOME=~/.kiro` opt-out — `kiro home: <path> (shared host home via KIRO_HOME; this instance reads the default instance's specs; its managed-spec rebuild never writes them)` (the guarantee is scoped to the guard's actual coverage — the managed rebuild and the app-spec bridge; editors acting on a direct user action follow `kiro_agents_dir()` and so edit the shared file under the opt-out, see [config](config.md#kiro-home-of-a-non-default-data-home)), after which nothing else follows — the home the CLI prologue adopted (`config.paths.adopt_isolated_kiro_home`). On an isolated home two further, purely informational `⚠`/`⏹` lines may follow, neither an `issues` entry: (a) `shared spec:` when the host `~/.kiro/agents/kirocrew.json` still pins THIS data home into a managed server (one of `mcp_cleanup.KIROCREW_BIN_MCP_SERVERS`; a user-added server inside the owned file is preserved by the rebuild and therefore not counted) — the leftover of this install writing the shared specs before it owned an isolated kiro home, which breaks the DEFAULT instance's sessions; the remedy named is `kirocrew setup --agent-only` from the default home (or restarting that gateway), because nothing on this instance rewrites that file any more; (b) `host home:` listing which of the host `sessions`/`steering`/`skills`/`prompts` still hold content this instance does not read, with `export KIRO_HOME=~/.kiro` as the opt-out (its Kiro Crew MCP servers then follow the default instance's specs). Read-only: the shared spec is read through `agent_discovery._read_agent_spec` (the hardened reader), the subtrees with one `is_dir()`/`any()` probe each, and an unreadable subtree is skipped rather than aborting doctor
21. **Agent Spec Data Home** (`doctor_spec_home.doctor_spec_home_drift`, right after the trust-root line, on the same agents dir doctor inspects): walks every `*.json` in the agents dir through the hardened reader and compares each `mcpServers.*.env.KIROCREW_HOME` PIN against the data home doctor runs on. Pins are compared LEXICALLY (`expanduser` + `normpath` + `normcase`, never `resolve()`): a spec is untrusted text and a UNC-shaped pin would otherwise open an SMB connection on Windows. A spec with no pin is not a finding (a default-home writer emits none). A pin that names another home on a MANAGED entry — an owned spec file (`agent_files.OWNED_KIRO_AGENT_FILES`) AND one of Kiro Crew's own server names (`mcp_cleanup.KIROCREW_BIN_MCP_SERVERS`) — IS an `issues` entry (`agent specs pin a different KIROCREW_HOME than this data home`, nonzero exit) with the remedy `kirocrew setup --agent-only`: sessions spawned from that spec verify their identity against the other home, so every strict-identity MCP tool is refused while the trust-root line above reads green — the #9690 shape this section exists to name. A pin on a FOREIGN entry (a third-party spec, or a user-added server inside an owned spec, which the rebuild preserves) prints the same `⚠️` line marked `foreign spec — not rewritten by setup; edit it by hand` and appends nothing: doctor's nonzero exit is a promise that something on THIS install is broken and fixable. Best-effort: an unreadable or malformed spec is skipped (the dead-path check reports those) and a failure inside the walk prints `could not check` rather than aborting the run
