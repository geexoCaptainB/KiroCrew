...
another unit's log consults rather than a panel. This module is those six folds, the
...
has not already consumed -- which is what lets one chunk serve six folds sitting
at different seqs.

## 3. The projections

### The panel folds and `class`

Each reads ONE session unit, and these six are what the growth push sends.
...
The TARGET's class is asked again after the entries are read; the caller's is not. A
read of another session's log is decided on the loop and then performed off it, and
that offload is a suspension point: the target can append a class move while the
entries are being gathered, and a payload returned on the earlier verdict would be a
one-shot delivery of content the refreshed class governs. The target's RECORDED class
is what gets folded again, and for that reason
...
The CALLER's class is deliberately not re-asked, and the reason is that such a test
could not buy an invariant. A caller's class says where its turn LANDS, and landing is
decided when the turn is emitted, which is after the route has answered. A caller that
acquires a channel link one instant after the payload returns, or that still holds the
payload in context on a later turn, publishes the same content with nothing to stop it,
and no refusal can claw back a read already granted. Re-asking would shrink one race
window by the duration of a read while leaving the adjacent instant open, and would
state in the source a guarantee the route cannot make. The caller's class is tested at
the GATE, where it decides whether the read happens at all. For the same reason a
LISTING is not re-checked: its scope is a filter rather than a verdict on rows, so the
only thing that could move under it is the caller's own class. The CREATOR EDGE is
another input not asked again: it
...
`COALESCE_SECONDS`, folds all six projections from ONE incremental read of the
