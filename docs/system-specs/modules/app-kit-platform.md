...
`agent.foreign_home_targets_shared_agents_dir(agents_dir)` through
`_shared_dir_owned_elsewhere`; when it names a home, the write is skipped
(`_register_agents` logs one WARNING and returns `[]`; the pruner and remover
return without touching the directory) and the refusal is SEL-audited exactly as
`agent._decline_shared_agent_home` audits its own: one `api_access` row,
`operation=agent_home_write`, `outcome=denied`, `source` naming the writer that
stood down (`register_agents`, `deregister_agents`, `prune_stale_app_resources`),
`resources` the shared directory. A permission decision about the machine-wide
agent home is reconstructible from the audit log alone, whichever writer made it.
The predicate names a home only when this process runs on a non-default
`KIROCREW_HOME` AND the target is the machine-wide shared directory rather than
the instance's own `isolated_agents_dir` — the `KIRO_HOME=~/.kiro` read-only
opt-out, or a caller that bypassed the CLI prologue's `KIRO_HOME` export (see
