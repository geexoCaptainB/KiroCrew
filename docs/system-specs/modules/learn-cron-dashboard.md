...
Member roster and rules validation reuse one off-loop configuration snapshot
for stable slug resolution and collision checks; per-member lookups do not reload
configuration on the request loop.
...
**`GET /api/lessons` returns one bounded window and says how much it left out.** The query takes `limit` (default `LESSON_LIST_LIMIT` = 50, clamped to `[1, LESSON_LIST_LIMIT_MAX]` = 500) and `offset` (rows skipped from the NEWEST end, clamped to `[0, LESSON_LIST_OFFSET_MAX]` = 100,000,000 because the vector tier binds it as a SQLite parameter and a value past the 64-bit range raises there instead of yielding an empty page); a non-integer is refused with HTTP 400 `invalid_pagination` rather than defaulted, because a page the caller cannot tell apart from the one it asked for is a wrong page. All three constants live in `validation.py` beside `LEARN_LIST_SCHEMA`, so the tool's advertised bound and the route's enforced one are one value. The body is `{"lessons": [...], "total": N, "truncated": bool, "limit": L, "offset": O}`: `total` is every live lesson in the store the caller is bound to (`count_lessons()` on the vector tier, the length of the global-plus-workspace union on the JSONL tier), `truncated` is `len(lessons) < total` — the body does not carry the population — and `limit`/`offset` are the values actually served, which is what makes the clamp non-silent. Rows inside the window stay oldest-first; `offset` walks back in time on both tiers (`get_lessons(limit, offset)` is `ORDER BY updated_at DESC LIMIT ? OFFSET ?` on the vector tier; the JSONL tier takes the window that ends `offset` rows before the tail of its append-ordered union). For V1 only, the vector-to-JSONL fallback is keyed on the vector POPULATION, not on the page, and the population test is whether any vector row DECODES -- a rendered row on the page settles it, otherwise `has_any_decodable_lesson()` scans the store -- not `count_lessons()`; the predicate is deliberately looser than context injection's `has_any_lesson()` because this list keeps every decodable row (legacy strings, rule-less mappings, marked withheld) so it stays deletable: an empty page past the end of a populated vector store answers `[]` with the vector total and never lists the superseded JSONL rows, while a store holding only rows an import or legacy migration left undecodable does not silence the JSONL file the caller's valid corrections still live in. `count_lessons()` sizes `total` only. A V2 member always answers from its sole SQLite database, including its `total`, even when empty or containing only undecodable rows; it never falls back to JSONL. A temporary session (`blocks_reads`) still answers the same shape with `total: 0`. The counts exist because this was the only lesson surface that omitted rows with no reader able to notice — injection reports `Showing n of total`, the memory graph reads the population, `kirocrew learn list` is unbounded — while `learn_list` rendered this body verbatim under a description promising "all", and `learn_add`'s `deduped` outcome sends the model to that list to find the stored lesson it lost to, which as an older dedup winner (a dedup never bumps the winner's `updated_at`) sits exactly outside the newest window. `learn_list` therefore accepts `limit`/`offset` (validated by `LEARN_LIST_SCHEMA` against the route's own bounds), prints `Showing N of M lessons; K older not shown -- pass offset=X to list them.` as its first line whenever the body is not the population (K is `total - offset - limit` and X is `offset + limit` -- the window the store consumed, not the rows shown: the route drops a row whose stored JSON does not decode, so a short page still cost `limit` rows, and counting by the shorter number would overstate the rest and re-read that tail; the offset hint is dropped on the last page, and an empty page past the end reads `Showing 0 of M lessons.` rather than "No lessons saved."), and its description no longer says "all". A page whose rendered rows would pass `MAX_RESPONSE_LEN` (where `sanitize_response` cuts the tail while the header at the head survives) is refused whole with a limit that fits, never shipped as a "Showing N" claim over rows the model cannot see. The dashboard Memory tab reads the default window and shows the tail 20 rows with no count derived from the list, so it is unchanged; its lesson count comes from the status snapshot's `count_lessons()`.
...
3. Otherwise the slot name (portion after the `:` prefix, or the whole key) must satisfy at least one of:
   ...
   - A dedicated subagent (`subagent:`) or headless workflow (`wf:`,
     ...
     or approval and stored in its canonical execution context. Active subagents use
     their gateway-owned snapshot; recovery reads the owning run record off-loop.
     Workflow workers use their captured execution context. The gate does not follow
     ...
     This existence check does not replace restricted-mode checks, ordinary
     transport/owner/app authorization or canonical store routing, **OR**
...
session. Its captured memory mode must permit the operation. Session recognition
and ordinary transport/owner/app authorization remain independent of memory
routing through the canonical execution context; there is no private-memory
process proof or separate grant registry.
...
Agent jobs capture an `execution_context` in the atomic schedule write. The
record distinguishes stable member identity from execution template and retains
...
errors retain their separate recovery guidance. At execution, the job's captured
...
Normal command/script permissions, application ownership and scheduler rules
remain independent. Context assembly uses the captured member, template and
project even when the memory database is unavailable; memory operations report the
database failure. Minimal context keeps required member/project guidance while
omitting optional context. Temporary execution reads no learning memory, and
restricted execution cannot persist learning from its work.
...
1. the **dashboard slot + bell** — `inject_cron_result_to_dashboard` (gated on `persistent_session and not hide_in_chat`; the tab is filed into the job's chat folder when it is minted, see **Run Filing into a Chat Folder**) then `notify`;
...
### Run Filing into a Chat Folder (`chat_folder_id`)

A SECOND, unrelated folder field (issue #1620). `folder_id` above groups the job's ROW on the Schedule page (`cron_folders.json`); `chat_folder_id` names a folder in the **chat sidebar's** own tree (`folders.json`) and decides where the job's `cron-{id}` TAB lands. A job may carry either, both or neither, and neither is ever derived from the other. Persisted on the job record, `""`/absent = not filed, capped through `_CRON_STRING_FIELD_CAPS`, accepted by `POST`/`PATCH /api/crons` and returned by `GET /api/crons`.

**What it is for**: a recurring job's tab otherwise sits loose in the sidebar. With a chat folder set, the tab lives in that folder, and its run stamps and run markers already make it the job's timeline.

**Persistent jobs only.** A stateless job (`persistent_session=False`) has no job-wide tab to file, so the store refuses the pair at save time (`_CHAT_FOLDER_NEEDS_PERSISTENT`, a `ValueError` the REST handlers surface as `400`) — on create, and on an update that itself names a folder for a job it leaves stateless — rather than accepting a setting with no observable effect. Turning persistence OFF on a filed job is not refused: the store clears `chat_folder_id` in the same update and fills the caller's transition sink with the prior folder exactly as an explicit clear does, so the dashboard handler moves the tab through the same save-time path. A refusal there would be a dead end, because MCP/CLI `cron_update` exposes `persistent_session` but not `chat_folder_id`, so that caller could never un-persist a filed job. The Schedule form disables the picker for such a job and says why (`chat_folder_needs_persistent_session`). A `script`/`command` cron never gets a tab either, so the form does not offer the picker for one and sends `""`. `hide_in_chat` SUSPENDS filing: the form disables the picker and says why while KEEPING the stored folder, since the runtime already ignores it for a hidden job and wiping it would make the form's own "turn off Hide in chat to use this" hint false. Per-run tabs for stateless jobs are deliberately out of scope; filing each run of a stateless job would be a follow-up.

**It supplements delivery, never replaces it.** Filing is a placement, not a fourth delivery mode: the notification, the Slack post and the origin-session injection all still happen exactly as before.

**A tab is filed by the call that MINTS it, and by nothing later.** `_bind_cron_slot` — the single creator for both the run-start pre-create (`ensure_cron_slot`) and the delivery — reads whether the slot existed BEFORE `get_or_create_slot` and schedules its filing (`persist_cron_chat_folder`) only when it did not. "Unfiled" is deliberately NOT the test: a reader who drags the tab out to the root leaves `slot.folder_id == ""`, indistinguishable from never-filed, so a rule that filed unfiled tabs would drag theirs back on every delivery. A freshly minted tab has no placement history, so filing it cannot undo anyone's. The synchronous path only DECIDES (`chat_folder_for_minted_tab`); the assignment of `slot.folder_id` belongs to the commit helper on the background task, because the delivery publishes the slot table synchronously right after the mint and a provisional value there would be broadcast before any write.

**Folder lifecycle.** A RENAME is a no-op — the job stores the folder's id, so it follows the folder under its new name. A DELETE leaves a dangling id, and the contract matches `folder_id`'s: a dangling id means "not filed". The run still delivers, the tab lands unfiled, and the skip is recorded ONCE (a log line plus a `cron_chat_folder_missing` SEL row with outcome `skipped`) so "why is this job not in my folder?" has an answer without a repro. **Existence is also checked at SAVE time**, where a person is present to be told: `POST`/`PATCH /api/crons` refuse an id naming no folder in `state._folders` with `400 unknown_chat_folder` (non-string or over-cap: `400 invalid_chat_folder_id`; `null` means "not filed").

**Persistence of the placement.** The folder's contents are read from the on-disk session list (`list_sessions` carries each session's `folder_id`), so an in-memory assignment alone would vanish on restart. Every placement — the filing of a minted tab and the save-time move of an existing one — commits through ONE helper, `_commit_cron_tab_placement`: it assigns `slot.folder_id` immediately before the write it feeds (`save_slot_off_loop` serializes the folder off the slot, with no override), asks for a confirmed verdict (`force=True, best_effort=False`; the returned bool decides, since a save can refuse without raising), pins the write with `expected_history_key` and `expected_slot_name` so a tab the reader closes and archives — or a same-name recreate — while the write is in flight refuses rather than overwriting the closed session's metadata, tells the sidebar about the placement only after the verdict, and rolls the tab back (to unfiled for a filing, to the prior folder for a move) when the write did not commit — unless the reader moved it meanwhile, in which case their placement stands. The filing runs on a tracked background task so no suspension point sits between a run's result and its notification.

**Changing the field** — clearing it, switching folders, or filing a job that was unfiled — moves the job's EXISTING tab at the SAVE (`move_cron_job_tab`, called by `PATCH /api/crons/{id}` after the store commits), under two conditions. First, a real TRANSITION: the store fills a caller-owned sink (`chat_folder_transition_out`, read under the same file lock as the write, so it is neither a read-then-write race nor shared across concurrent updates) with the PRIOR value — `""` included — only when the update actually changed `chat_folder_id`; the Schedule form sends the field on every submit, so the request alone is evidence of nothing. Second, the prior folder is checked AGAINST THE TAB: it moves only when it sits where the setting last left it. A reader who dragged it elsewhere has said where they want it, and their placement outranks a setting they are editing.
...
- **Script path resolution and its two roots**: `resolve_script_path` (`cron_script.py`) is the single resolver for every script spec, and its DEFAULT is the operator contract: a relative path resolves against the process CWD and the resolved file must sit under `~/.kiro/crew/crons/`. `cron_add`, the `cron run-script` CLI and both vault-grant sites take that default, so an authoring caller can never name a file outside `crons/`. An app cron's script legitimately ships inside the app's own bundle instead, and two opt-in keywords cover that, each opening exactly one root. `app_root` is passed where a MANIFEST's own spec is vetted (`apps.bridges`, `apps.cron_sdk`): it is the base a relative spec resolves against, because `"job.py:run"` means "next to my manifest" and is the only spelling an app can write without knowing its install location -- resolving it against the gateway process's CWD instead produced a path that changed with every launch and named a file that was never there (#11819) -- and containment narrows to that ONE bundle, so app A cannot name a script in app B's tree. `allow_bundle_roots` is passed only by the three consumers that re-resolve a spec ALREADY vetted and persisted while holding no app context (the fire-time governance gate, the launcher, the dashboard script-source endpoint); containment is then the shared bundle roots -- the installed `kiro_crew` package, which is where a shipped builtin's bundle lives, and `<config_dir>/apps` -- because a stored absolute bundle path is all those callers have to go on. The app bridge and the cron SDK therefore PERSIST the resolved absolute spec rather than the manifest's relative one: a relative value on the job record would be re-resolved against each consumer's own CWD. A bundle root accepts `.py` files only, which is containment and not style -- a bundle also holds `.app_secret` (the app's gateway credential) and `data/`, and a resolved script's bytes are readable through the script-source endpoint, so the suffix keeps every non-module bundle file off that surface; `crons/` keeps no such rule because it holds nothing else. A `..`-bearing relative spec is refused lexically before any join so the verdict never depends on the host's path grammar, and `resolve()` runs before containment so a link pointing out of a trusted root is rejected on its target. The vault-grant reader (`_read_script_body`) stays pinned to `crons/` alone and the grant paths pass neither keyword, so a bundle script can register and run but can never be handed a secret.
...
- `GET /api/tasks/summary` — `{generated_at, available, depth{by_state,
  ...
  ladder[…]}, stalled, counts, stall_after_secs}`. `depth.queued` sums
  `session_health.TASK_QUEUED_STATES` (`queued + admitted + retry_wait +
  waiting_infra`) — the constant itself, shared with `/api/sessions/health`'s
  `queued.count`, never a second list here. `depth` is a set of VIEWS over the
  rows, not a partition of them: `retry_wait` counts under both `queued` (it
  waits for capacity) and `recovering` (it is being retried).
  **Every `depth` number is counted off the one uncapped `count_by_state()`**,
  `depth.recovering` (`recovering + retry_wait`) included, so no two numbers in
  one response can disagree about how many rows there are. The ROW LISTS are
  pages: `waiting` comes from the uncapped `active_rows()`, but `retry_wait`
  holds no runtime and so is read by a `list_rows` bounded at `_LANE_SCAN_LIMIT`
  (5000) — and counting `depth.recovering` off THAT would make a page size a
  silent cap on a count, which the card cannot survive, because it computes
  "waiting for a slot" as `depth.queued` less the uncapped
  `by_state.retry_wait`: every row past the page would then appear in neither
  number. Pinned by
  `test_api_tasks.py::test_every_depth_number_counts_the_same_rows_as_by_state`,
  which shrinks the page rather than the volume. `lanes` merges the health monitor's registered cap
  ...
  `recovering.task_attempts` sums `attempts` over the `recovering`/`retry_wait`
  rows THAT PAGE CARRIES, so it is an attempts figure for the rows on the wire
  and never a second answer to "how many rows"; `recovering.ladder` is
  `default_ladder().table()`. `available` is
...
**Store prose is redacted on the way out, on every route above.** A row's wait
`reason` and an event's `data` carry a failing tool's, subagent's or dependency
adapter's own text — a `finish(error=…)` detail, a GitHub stderr tail, an HTTP
body — which the store keeps VERBATIM so the row stays diagnosable after the
process that failed is gone. `handlers/tasks._scrub_prose` runs
`security.redact_and_truncate` over every string leaf, walking nested dicts and
lists (a credential inside a signal dict is the same leak), which is the
composition `subagent.py`'s `_redact` and `taskrunner._execute_tasks` spell for
the same strings: **exfiltration URLs first, then credentials.** That order is
load-bearing — a URL whose query IS the credential is what the URL rewriter
exists for and it removes the whole URL, while credentials-first leaves
`?token=[REDACTED: credential]`, in which nothing suspicious remains, so the
destination survives. Non-string leaves and mapping keys pass through (a number
carries no credential; a key is a field name the writers spell in code).
Identifiers — `id`, `lane`, `session_key`, `lease_owner`, an event `kind` — are
served as stored: the panel keys its rows by them, and rewriting one would break
the match. The list and detail routes build their payloads INSIDE the
`asyncio.to_thread` that reads the rows, because a redacted `limit=1000` page or
a 200-event tail costs about as much as the read itself and the loop is shared.
Pinned by `test_api_tasks.py::test_task_routes_redact_store_prose`
(a real store, a real handler, both shapes, nested) and
`::test_redaction_runs_exfil_before_credentials`.
...
- `POST /api/tasks/{task_id}` — one action on a TaskRunner / workflow row:
  ...
  **The state tests here are pre-checks; the fence is the writer's.** Both arms
  ...
  returned AND over the one state the action answers —
  `cancel(only_from=taskq.model.PARKED, generation=…)` for `cancel_wait`,
  `wake_wait(only_from={waiting_input}, generation=…)` for `answer_input`
  (`taskq.md` § A cancel whose PRECONDITION came from an earlier read). **An
  answer answers ONE question**, so the wake predicate is `waiting_input` alone
  and not the whole `WAITING` set: a row that has finished that question and
  parked on a dependency, or on its children, is in a DIFFERENT wait, and ending
  that one with an answer to a question already over would report 200 and leave
  the row claimable with its dependency still unmet. A refusal
  ...
  or whose row is in another wait.
  ...
  pins that every PARKED state still cancels through the real route. The answer
  arm's own race is
  `test_api_tasks.py::test_answer_input_is_refused_when_the_row_moved_to_another_wait`:
  the row re-parks on a dependency under the handler's read, through the store's
  own writes, and the route answers 409 with the dependency wait intact.

  **The release reaches the parked coroutine, and the route's thread is not the
  loop's.** Both arms run in `asyncio.to_thread`, so `RunnerAdmission.on_wake` —
  the only thing that sets the `asyncio.Event` a parked run awaits — is entered
  on a worker with no running loop. `Event.set` is a loop operation (it resolves
  the waiter's future, which schedules that future's callbacks), so the set is
  posted to the loop the waiter parked on with `call_soon_threadsafe`, the shape
  `config.live.LiveConfig.notify_written` uses for the same reason. Making it in
  place would be the cross-thread call asyncio refuses under `loop.set_debug`,
  and the operator's stop would leave the run waiting for a wake already spent;
  pinned by
  `test_taskq_runner_adapter.py::test_an_operator_action_from_a_worker_thread_releases_the_parked_run`.
...
label/value shape — Queue ("Waiting for a slot" = `depth.queued` LESS the
`retry_wait` rows `by_state` reports, running, "Paused mid-run", "Tasks
retrying" = `depth.recovering` (task rows re-dispatched after a stall plus rows
parked in `retry_wait`), oldest wait), Capacity per lane (the cap in force
leads every lane — the bare "Up to N at once" while the lane is not under the
user's ceiling, and one line carrying both numbers when it is — then the cause
where this lane is its home, then the live count ("N running now") — never
`N of M`, which beside the running line reads as two answers to one question),
Recovery ("Retry attempts" = `recovering.task_attempts`, "Backends restarting" =
`recovering.slots` (live chat/run sessions restarting after a stalled turn —
sessions, not rows), stalled) — then one list, stalls first and longest wait
first within each group, of every waiting/retrying task row, every stalled slot
and every other non-running live slot with its state badge, id, reason, attempts
(when >1) and age, bounded at 8 rows by a fold that OPENS IN PLACE (a "Show N
more" / "Show fewer" button carrying `aria-expanded` and `aria-controls` on the
list): a count of rows the reader cannot reach is a dead end, and the bounded
default is what keeps a 2000-row queue from becoming the whole card.
...
no second number to reconcile) and `effective < user_max` puts BOTH numbers in ONE
line, joined by the verb that relates them ("Up to 4 at once, lowered from
your 14"), plus the degrade reason itself when that reason is one of the mapped
`adaptive_*` tokens. A bare cap stacked over a line that calls the ceiling "your
limit" states two limits, and a reader asks which of them is in force; one line
makes the second number the ORIGIN of the first, and the cause beneath it
continues the same verb. `cap_ceiling` ("Your limit: N") is left for the wire
contradiction alone, a cap ABOVE the configured ceiling, where neither of those
statements is true. The ceiling is glued to the word before it with U+00A0 in
every locale that can break there: a wrapped line leaving a bare "14" under
"lowered from your" puts back the loose second number this line exists to remove.
The bare "Up to N at once" is the ONE line this column keeps from wrapping, since
it is short; every line that says more than the number is prose, set in the UI
font and free to wrap, because a phrase that cannot wrap at the ~230px a column
gets lands on top of the column beside it. The lead line is never muted whichever
shape it takes — it is the answer the column exists for.
...
this", and the stalled COUNT in the Recovery column, with its row in the list, is
where a stall is reported — and absent again when the single lowered lane already
...
Every counter label is DISJOINT from every other one, and so is the SET each one
counts, because the reader has no tooltip open while comparing them: "Waiting for a
slot" is accepted work holding no runtime and not being retried, "Paused mid-run"
is a run that already started and gave its slot back, "Tasks retrying" is queue
rows, "Retry attempts" is the attempts those rows have spent, "Backends
restarting" is sessions, "Stalled" is slots making no progress with no wait reason
that explains it. Three of those numbers are computed BY THE CARD rather than read
off the wire, because the wire's own fields double-count or undercount against
that rule:

- `depth.queued` sums `queued + admitted + retry_wait + waiting_infra`, and
  `depth.recovering` counts those same `retry_wait` rows as work being retried, so
  the card SUBTRACTS `by_state.retry_wait` from "Waiting for a slot". A row parked
  until its next attempt is retrying — the more specific fact, and the one its own
  list badge ("Waiting to retry") states — so it is counted under "Tasks retrying"
  alone. `queued_tip` states that rule; a tip that confessed the overlap instead
  would leave the two numbers unaddable on the surface where they sit side by side.
- "Paused mid-run" is counted off the list's OWN entries (the `waiting_*` ones),
  not off `depth.waiting`: the wire field counts store rows only, so a live chat
  slot parked on the operator's approval was a list row that no counter admitted to.
- "Stalled" is counted off those same entries, so the count and the row it points
  at come from one array.

The card therefore reconciles — list rows == Paused mid-run + Tasks retrying +
Backends restarting + Stalled — for as long as the summary's `retry_wait` page
holds every such row, which is the only way any of these counters and the rows
beneath them can differ: "Tasks retrying" is `depth.recovering`, counted off the
uncapped `by_state`, while the list carries at most `_LANE_SCAN_LIMIT` of those
rows. Past the page the COUNT stays the truthful one and the list is short of it,
which is the right way round — the alternative caps a count, and then a queue
larger than the page reports rows in neither "Waiting for a slot" nor "Tasks
retrying".
...
The four retry surfaces name four disjoint things in that ONE vocabulary: "Tasks
retrying" is the rows, "Retry attempts" the attempts those rows have spent, the
two badges ("Retrying now", "Waiting to retry") the two states inside that count,
and "Waiting for a slot" the accepted work that is NOT among them — which is why
the retry-wait subtraction above is what makes the vocabulary answerable rather
than merely consistent. The keys stay `recovering`, `task_retries` and
...
**A stall always has a row.** It is the one condition on this card that nothing
recovers on its own, so the number that raises the alarm must be openable. The
card reads ONE set — the `stalled` map's keys unioned with every slot whose own
`classification` says stalled (`stalledKeysOf`) — and the badge, the "Stalled"
counter and the list rows all come off it, so the card can neither go red over a
stall with no row nor list a row no counter admits to; the key is skipped in the
slot loop, so one stall is one row. That row's reason column carries the stall's
EVIDENCE rather than its `reason` field, because `session_health` files
`no_progress` there for every stall the structured view classifies — the badge
already says that — and puts a sentence there only from the log-scan arm, which
this endpoint never runs; the `reason` is therefore the fallback for a stall with
no evidence. A stall renders no "Open chat" link (nobody is being waited on, and
the card is read-only) and sorts AHEAD of the waits: the fold shows the first rows,
so ordering by age alone can hide the one row the red badge names behind it, and a
five-second stall is still the alarm where a two-hour approval wait is not.

A healthy / degraded badge sits in the title, and the late-queue one is "Backlog":
"Backed up" is first read as a backup copy. Its rules read the numbers the card
SHOWS, never a wire field the card presents differently — the depth arm counts the
slot waiters the Queue column prints, not the raw `depth.queued`, because an amber
"Backlog" over a queue count below its own threshold is a verdict the card
contradicts on the same line. **No badge sits there at all when no
verdict is supported.** The stalled and degraded rules read the live session
monitor and the controller, which a gateway with no task store still publishes, so
those two verdicts stand on any install; every input the healthy and backlog rules
stand on (`depth`, `oldest_wait_secs`) is a zero such a payload has nothing to fill
with, and a green "Healthy" over the store notice is then a health claim about a
queue nothing is recording — indistinguishable, to the reader, from "there is
nothing to show". So `cardHealth` answers null there and the notice is the only
statement. Empty state is one line ("Nothing
...
UX evidence: `website/scripts/capture-tasks-capacity.mjs` photographs the real
built card, one scene per branch it colours or explains — including BOTH homes of
the degrade reason (the row when two lanes are lowered, the lane line when one is)
and both arms that mount no health badge — and refuses to write a frame whose
title row or badge is clipped or painted over.
...
**Project git**: GET `/api/project/git?path=<dir>` (checked-out branch label for the activity panel). `path` is matched against the gateway's own known project directories (every live chat slot's `project` plus the recorded recent-projects list) using pure string normalisation (`expanduser` + `normpath`), with no filesystem access on the untrusted value; an unrecognised directory is refused with 403 plus an SEL audit record, and only the matched server-held string is stat'd. Response is `{"repo": false}`, `{"repo": true, "repoRoot", "branch"}`, or the detached form `{"repo": true, "repoRoot", "detached": true, "head"}`, plus `path` on the envelope. No `git` subprocess is spawned; the branch comes from reading `.git/HEAD` directly (a `git` invocation parses repo-local config, which can carry `[include] path = <file>` and read an arbitrary file, so reading HEAD ourselves removes that class), and a linked worktree's `.git` pointer file (`gitdir: <path>`) is followed to the real HEAD. Both metadata reads go through `hooks.safe_read_prefix` (realpath canonicalisation, sensitive-target rejection, `O_NOFOLLOW`, bounded read). `branch`, `head`, `path`, and `repoRoot` all pass through `redact` before being returned; the SEL audit keeps the real path. Every failure mode degrades to "no branch" rather than erroring, since the only consumer is a decorative label. A detached HEAD gets a fixed 7-char prefix, not git's dynamic abbreviation. GET `/api/project/git/status?path=<dir>` and GET `/api/project/git/log?path=<dir>` use the same known-project boundary. The status route first checks `os.path.isdir` and repeats that check after any subprocess failure; a missing or non-directory project returns `200 {"repo": false, "files": []}`. It performs no in-process inspection of `.git` or anything `.git` points to. Instead it runs sandboxed Git with `LC_ALL=C` and `LANGUAGE=C`: a bounded `git rev-parse --git-dir` probe owns repository discovery. A nonzero probe whose bounded stderr contains Git's `not a git repository` verdict is confirmed absence and returns `200 {"repo": false, "files": []}`. Symlink markers, gitdir files, parent-repository discovery, ceiling and cross-filesystem rules therefore follow Git. Any other probe failure, including sandbox refusal, dubious ownership, permission failure, timeout, kill, or missing executable, returns 503 with `code: "git_status_unavailable"` unless the project directory disappeared. After a successful probe, a nonzero `git branch --show-current` (which validates corrupt symbolic HEAD while accepting detached and unborn states), a failed or empty `git rev-parse --show-toplevel`, or a nonzero `git status` is also 503. **The filter-driver refusal is 503 too, not an empty listing, and it carries its OWN code.** A repository whose own config names a `filter.*.{clean,smudge,process}` driver -- anything `git lfs install --local` touched -- is refused because `git status` re-hashes modified files through a `clean` driver on every 5 s poll, and the probe refuses under its own separate cause when it cannot prove a driver absent. The match is deliberately WIDER than execution: `smudge` fires on checkout rather than on the status re-hash, a driver that no `.gitattributes` path maps to never runs at all, and the probe-failure branch refuses without knowing a driver exists. Refusing all three is right, since none of them can be proven safe -- but the user-facing message therefore says the config DECLARES a driver and that policy refuses the check, and never claims a program ran. Spelling that refusal `200 {"repo": true, "files": []}` makes it byte-identical to a genuinely clean repository, so the panel draws its green clean badge over a working tree whose status was never read; the panel's one job is surfacing uncommitted changes, and a confident "none" is worse there than no answer at all. The code is `git_status_filter_refused`, distinct from the `git_status_unavailable` its four siblings use, because this condition is not an outage: the cause is knowable, and for the `declared` cause it is permanent for that repository with no retry clearing it while the config stands (the `unreadable` cause is the one that can clear on its own, which is why only the declared copy promises permanence). One code for both would tell an LFS user their repository is broken on every poll forever. The panel renders it as a SINGLE `ErrorNotice` whose message names the cause, rather than the two generic notices the outage path shows: both routes refuse together because the cause is repo-level, so two boxes would report one condition twice. The refusal notice is TITLED and the outage notice is deliberately untitled -- titling both put two red boxes opening on the same word beside each other while they carry opposite retry semantics, so the difference between permanent and transient was a body-read away; titled against untitled separates them at a glance, and the outage title was restating its own first clause anyway. The refusal title names the OUTCOME ("Kiro can't check this repository for changes") rather than the mechanism: "status checks are off" collided with the outage string's "couldn't read the repository status", so one panel used the word status for two conditions of different severity. A coded status refusal reaching the non-coalesced branch renders that same title and localized copy, because the backend's English sentence under a localized heading is mixed-language copy in the other twelve catalogs. The header REFRESH control is also inert while BOTH routes refuse, and stays live for an outage: the refusal copy promises that retrying will not help, a re-fetch returns the same 503 every time, and a live control over that sentence contradicts it. It is disabled rather than hidden so the header does not reflow, and the glyph itself switches to the slashed variant, because dimming plus a hover tooltip did not tell a reader the control was dead. It is keyed on both routes rather than on the coalescing predicate deliberately: the refusal is permanent only while the repository config stands, and the notice's own agent hand-off invites changing it in the same window. The status query then recovers on its own 5 s interval while the log query -- no interval, 30 s `staleTime`, and a branch-change effect that cannot fire on the FIRST successful status because it needs a previous marker -- still holds its cached refusal. Keying the inert state on the coalescing predicate would therefore disable the one control able to clear that, leaving a notice claiming history cannot be shown while the changes list renders beneath it. The panel also refetches the log automatically as soon as the status route comes back CLEAN (not merely stops refusing: a status route failing for its own reason proves nothing about the config). The 503 body carries a `cause` discriminator alongside the code -- `declared` when repo config names a driver, `unreadable` when the probe could not prove one absent -- and the panel renders one localized sentence per cause. One code, because it is ONE condition (refused by policy) with two causes; a disjunction in the copy ("declares a driver, or could not be read") made the common case, an LFS repository on every visit, read a question the guard had already answered, and a reader could only partly follow it. Naming a driver as fact in the probe-failure branch would still be wrong, which is why that branch answers its own cause rather than borrowing this one. It stays an `ErrorNotice` with the `askAgent` hand-off because the string originates in a `{error, code}` body, which is what the `errors-use-error-notice` rule scopes on, and because a repo-supplied filter driver is precisely what the agent can explain or change -- routing it around that surface would strip the one affordance that helps here. HEAD is still validated BEFORE the filter guard, which `test_status_checks_corrupt_head_before_filter_refusal` asserts by spying that the guard is never reached, and the two codes now witness the ordering as well. A successful listing is capped at 500 entries and carries `truncated: true` when entries were dropped; the panel renders that as a capped count (`500+ uncommitted`) plus a `showing first 500` note on the Changes header, because without it the cap read as the total and an undercount was presented as a count. `/api/project/git/log` applies the same filter-driver refusal and answers it with 503 `code: "git_log_filter_refused"`, since an empty commit list is what a repository with no commits legitimately returns. A nonzero `git log` deliberately stays `200 {"repo": true, "commits": []}`: an unborn repository exits 128 there, so that path is genuinely ambiguous and separating it would need the stderr match the status probe uses. The Git panel treats only `repo: false` without a simultaneous status error as a terminal no-repository state: it replaces the loading label, hides the clean badge and clean-repository empty copy, and tells the user to click the folder name below the message box. The other three `['git-status', …]` consumers now receive 503s on operational failure where they got 200s, and each was checked rather than assumed. `FileBrowserRail` renders an inline `ErrorNotice` with the agent hand-off on an error in All mode, and deliberately not in Changed mode because the Pierre tree owns the same query failure there. `PierreWorkspaceTreeImpl` treats a status error as terminal for a Changed-mode load and renders that notice with a structured report. Both of those notices RECOGNISE the refusal codes and render the panel's own refusal copy rather than their generic failed copy, because the refusal is permanent for an LFS-configured repository: generic copy there would say "failed" on every 5 s poll, forever, with no cause and no statement that retrying cannot help — the exact outage spelling this refusal code exists to end. The recognition lives once in `website/src/utils/gitStatusError.ts` (`gitErrorCode`, `isGitFilterRefusal`, `gitFilterRefusalCause`, `gitFilterRefusalCopyKey`) and all three components read it, so one condition has one wording rather than three to keep in step. Neither of those two carries the panel's TITLE, deliberately: `ErrorNotice`'s `inline` variant lays title and message out as flex siblings, so at the rail's 300-520px and the tree's width the title wraps into a stack of two-word fragments beside a narrow column of message — visible in `capture-git-filter-refusal-surfaces.mjs`'s frames, which is what settled it. The title exists in the panel to separate a refusal from a coexisting outage notice, and neither of these surfaces renders a second notice beside this one. The status outage string also has a variant used when the log route failed too, which names the half it covers ("Couldn't read the changes list."): the default wording promises a commit list that is still on screen, so beside a notice saying history cannot be shown the two boxes first contradicted each other and then, once the clause was merely dropped, read as one failure reported twice. Naming each half is what separates them. The ChatPage footer badge is the only silent one, and correctly so: it is decorative, so `!projectGitStatusError && …` hides it rather than showing a broken count. None of the three reads non-200 as "not a repository"; all three read `repo` only from a 200 body, so a refusal reaches them as absent data rather than as a negative answer.
**Project tree**: GET `/api/project/tree?path=<dir>` (workspace file listing for the Files tab's Pierre tree, and for a chat side-panel `folder` tab whose path IS the chat's project root — `FolderPanel` renders the same `PierreWorkspaceTree` there so descending expands in place instead of re-targeting the tab, and falls back to its one-level `browseFiles` listing for every other directory and whenever this endpoint does not answer, because a directory path chip can name any directory on the gateway while this allow-list deliberately does not). File tabs start with their optional project-tree rail hidden so the document receives the full panel width; the toolbar toggle and persisted `mc-files-rail-open` preference keep it available on demand. Same known-project allow-list, sensitive-path refusal, and SEL auditing as `/api/project/git`. Inside a git repository the listing is `git ls-files -z --cached --others --exclude-standard` with `cwd=<dir>` (tracked + untracked, `.gitignore` honored; a project dir that is a repo subdirectory lists only its own subtree); outside one it is a complete `os.walk` that skips hidden and heavy directories (`_PROJECT_TREE_SKIP_DIRS`). Response `{root, paths: [<project-relative POSIX file paths>], directories: [<project-relative POSIX directory paths>], repo, truncated?, truncatedDirectories: [<direct parent directories whose files were omitted>]}`. `paths` carries at most `_PROJECT_TREE_MAX_ENTRIES` (10,000) files, allocated round-robin across their direct parent directories; `directories` carries the complete uncapped directory skeleton so a folder row never disappears at the file limit, including empty directories on the filesystem-walk path. The frontend passes those directories to Pierre as explicit trailing-slash paths, keeps the workspace-level truncation notice, and marks each directory named by `truncatedDirectories`. `truncated` remains true exactly when files were omitted. Older clients may ignore both additive arrays. `root`, every file path, every directory path, and every truncation marker pass through the same segment-aware redaction before egress. Lazy expand-on-demand loading remains the long-term alternative when a complete directory walk itself becomes too expensive.
...
finite provider-error budget. Because that budget is finite and never refunded, it
counts refusals the HOST gave this watch and nothing else: a tick that sent no
request is a third outcome (`github_provider_errors.is_unattempted_probe`, marked
by the `provider_rate_limited_shared` reason code a GitHub monitor sets when it
declines its own probe on the shared `github:api` schedule) and it moves neither
the cumulative count nor the consecutive streak. Charging it would retire a
healthy watch on the cadence of work that never touched its subject, and clearing
the streak instead would be the opposite defect — a provider failing on alternate
ticks with a cooldown between would blind the watch for ever. BOTH counting sites
separate the three, and they have to: `shadow.py::apply_monitor_probe` is the
shadow path, and `autonudge.AutoNudgeService.apply_monitor_probe` is the one a
wired controller drives, so a rule held in only one of them is a rule held
nowhere. Pinned on each — `test_monitoring_github_shared_cooldown.py` for the
shadow path, `test_monitor_controller.py::test_a_shared_cooldown_skip_does_not_retire_a_healthy_watch`
for the production one — and each pin also asserts that a refusal the HOST gave
still retires the watch on schedule, so the skip cannot be widened into a way of
surviving a real outage. This substrate does not itself schedule live provider probes or
...
**Lessons**: GET `/api/lessons`, POST `/api/lessons` (add), DELETE `/api/lessons` (remove by substring; `exact: true` narrows the rule match to the whole text, case-insensitive and whitespace-trimmed, for a caller that holds the full rule -- the Memory tab's row Delete -- so "use tabs" does not also take "always use tabs"; a non-boolean `exact` is a 400 `exact_not_bool`). A lesson's identity is the pair `(rule, repo_scope)`, so every row the GET returns carries the `repo_scope` DELETE selector that names exactly that row: `""` for a global row (the route's explicit-global selector), the canonical fragment for a scoped row, and `null` for a row whose stored scope is present but unusable -- both stores keep such a row reachable only through the UNSELECTIVE delete and the route refuses the raw value as a selector, so the client sends none for it. A stored fragment the shared credential redaction chain would alter is reported `null` too, as is one containing any control character (an escape sequence could split a credential so the chain passes it whole and a renderer reassembles it): the selector must round-trip byte-exact so it cannot be emitted redacted, and it is never emitted raw. On the JSONL tier each row also names the file it was read from (`scope`: `global` / `workspace`, plus `workspace` for the latter), because the list is a union of the global file and the active workspace's while the delete route defaults to the global file; the client forwards both so a workspace row's delete lands on its own file instead of removing a same-text global row; every workspace row is listed even when a global row shares its text, since the tier tells them apart and a hidden row is one the UI can never delete. The Memory tab renders that value as a Scope column and passes the clicked row's own selector to the DELETE (`client.ts` `deleteLesson(rule, repoScope)` sends the key only when it is a string), so deleting what looks like a duplicate row no longer removes its same-rule sibling in another scope (#10651). The Scope cell labels the global row ("Global") and the unusable-scope row ("Unusable scope") rather than leaving either blank, and the unusable-scope row's Delete asks for confirmation first through the shared `useConfirm` dialog (confirm button restating "Delete in every scope"), because it is the one delete this tab sends unselectively. A rejected DELETE, or a rejected list re-read after a successful one, renders an `ErrorNotice` above the table titled for the step that failed (no agent hand-off: the Add row may hold a draft) instead of ending in silence. The MCP `learn_list` and CLI `learn list` surfaces render the same scope beside each rule (`(scope: <fragment>)`, `(scope: unusable)`, nothing for global) so the three list surfaces and their scope-selective removes agree on a row's identity; `learn_list` also marks a row read from the active workspace's JSONL file with `(workspace: <name>)`, and `learn_remove` accepts optional `scope` / `workspace` (validated by `LEARN_REMOVE_SCHEMA`) forwarded to the DELETE body, so the model can name that row instead of the delete defaulting to the global file. The pair is validated together on both the tool and the route: `scope='workspace'` without a name is refused (400 `workspace_required`; it would otherwise fall through to the active workspace's or the global file), a name without the tier is refused (`workspace_without_scope`), an unknown tier is `scope_not_allowed`, a malformed name is `workspace_invalid`, and the reserved name `default` under `scope='workspace'` is `workspace_reserved` (it is the global file's name inside `_get_lessons`, so accepting it would route a workspace-tier delete onto the global rows), and a name absent from the configured `workspaces` map is `workspace_unknown` (`workspace_dir_for` resolves an unmapped name onto the DEFAULT workspace directory, so a typo would delete from that file) -- nothing is deleted on a refused pair. The Memory tab's Scope cell shows a workspace-file row's tier as a second line ("Workspace <name>").
...
actionable reason in the exact member's cached outcome, including member identity
and database availability errors. A failed repair keeps the confirmed
...
**Crew memory-store bindings**: explicit member creation provisions an empty
member-scoped V2 store. Discovery sync registers ordinary agents with Global V1
and preserves existing bindings. Existing V1 members keep their current memory;
editing a member offers no provisioning or migration action. Invalid or mismatched
member-store identities produce a named error and never inherit Global V1. See
...
**Pierre worker terminal diagnostics**: each failed generation records a classification (`error`, `messageerror`, `postMessage throw`, or `init timeout`), a bounded one-line message, its generation, and its attempt in `WorkerPoolSnapshot.failure`. The final failure remains on the terminal `unavailable` snapshot. The passive `ErrorNotice` renders that detail, so its Ask the agent handoff carries the same facts. Terminal exhaustion emits one structured `console.error`; the desktop main-window `frame-load-log` bridge copies renderer errors into `gateway-launch.log`, which puts the cause in the desktop diagnostics bundle without adding a browser-to-gateway endpoint. Browser-only sessions retain the same record in their console.
...
- `dashboardSlice`: SSE/WS connection state, chat slots array, approval mode (synced from backend `yolo` field in status — no localStorage), optimistic slot add/remove reducers (`addSlotOptimistic`/`removeSlotOptimistic`), async thunks for slot fetch / approval mode change. A slot created through `chatSlice.createSlot` is registered by a matcher on that thunk's own `fulfilled` action (idempotent by key, since the live `slots` frame usually announces it first), so the sidebar row and the activated empty transcript commit together rather than as a separate optimistic dispatch followed by the activation. **Close tombstone** (`closingSlots`): `chat/deleteSlot/pending` arms a per-key hold before the thunk's `removeSlotOptimistic` runs, and both authoritative list writers (`sseSlots`, `fetchSlots.fulfilled`, via `applySlots`) filter a held key out of the applied list — the server keeps a closing slot in its registry across the nudge-retirement / app-hook / history-save awaits before popping it, and coalesced pushes re-serialize at delivery, so a frame that still lists the slot is ordinary and used to put the row back until the post-pop frame removed it again (#11224). A list that OMITS the key does not release the hold: the two writers are unordered with each other (a `fetchSlots` reply can be older than live frames that arrived while it travelled), so a post-pop frame releasing the hold would let a pre-pop reply re-add the row. The tombstone is retired by the close's own lifecycle instead — released by a same-key `addSlotOptimistic` / `createSlot.fulfilled`, and on failure: the thunk's catch dispatches `releaseCloseHold` BEFORE its recovery `fetchSlots` (the `rejected` action also releases, but it trails the thunk's unbounded `await navigation`, so the refetch reply could otherwise land while the hold still filtered the very row it exists to restore). The hold is bounded twice over: `deleteChatSlot` carries no deadline, so the thunk dispatches `confirmCloseHold` the moment the DELETE resolves — not on `fulfilled`, which trails its unbounded `await navigation` — so a successful close whose peer transcript load outlasts the in-flight cap is not expired as if it had stalled; while the request is in flight the hold expires on the wall clock (`CLOSE_IN_FLIGHT_MAX_MS`, 30 s — a stalled server-side close degrades to the pre-fix "row comes back", not to a session hidden until reload); after `fulfilled` it outlives `CLOSE_CONFIRMED_GRACE_FRAMES` further authoritative lists (listing the key or not) AND every `fetchSlots` request that was already in flight at the 200 (`slotFetchesInFlight` snapshotted onto the hold's `awaitingFetches`; a pre-pop HTTP reply can trail the pop by a whole round trip, which no frame budget bounds) — the confirmed phase is itself wall-clock capped (`CLOSE_CONFIRMED_MAX_MS`, 30 s) because `chatSlots()` carries no deadline and a stalled fetch must not pin the tombstone until reload; the fetches still awaited at expiry are recorded in `staleSlotFetches` (request → closed keys) and on arrival, once the stream is live, only those keys are distrusted in the reply (the on-screen row is kept for a key recreated meanwhile, otherwise the key is dropped) while the rest of it still applies — then yields to membership, so a server that keeps listing a slot it confirmed closed cannot hide a live session indefinitely. Each hold records its `deleteSlot` `requestId`, and `fulfilled` / `rejected` / `releaseCloseHold` act only on the attempt that owns the hold, so a stale `rejected` trailing a slow peer navigation cannot release a same-key retry's hold. The other two `removeSlotOptimistic` dispatch sites (`switchSlot`'s 404 eviction, the artifact companion-chat close) remove only after the server has already answered and carry no tombstone; they are exposed to the post-confirmation straggler window alone (#11255). `reconcileSlots` still sees the frame's full membership, so a held slot's sub-agent state is not evicted before the close is confirmed (a failed close leaves the slot live). **Per-slot write recency** (`slotWriteSeq` / `slotWrittenAt` / `slotFetchWriteMark`): `applySlots` is a whole-list positional replace — a row survives only while it is `jsonEqual` to the incoming one — so a single-slot write that landed after the server serialized its `/api/chat/slots` reply and before `fetchSlots.fulfilled` ran was overwritten by the older server row (#11149). Every single-slot writer therefore goes through ONE chokepoint, `patchSlotRow` (and its URL-keyed sibling `patchSlotRowsWhere` for `patchSlotSourceLinks`), which stamps the row with a monotonic `slotWriteSeq`; `fetchSlots.pending` snapshots that counter per requestId, and on `fulfilled` any key stamped ABOVE its request's mark keeps the on-screen row instead of the reply's — the same substitution the `staleSlotFetches` close pairing performs, one level finer, and the same rule `notificationsSlice` applies to an ack flag with `ackSeq`. Deliberately NOT gated on `slotsLoaded`, unlike that close filter: `sseConnected` clears the flag on every reconnect while LEAVING the rows in place, and reconnect is the one caller that refetches while the WS replay backlog writes into those same rows, so gating would disable the guard exactly where the race is likeliest. A cold boot needs no gate — `patchSlotRow` can only stamp a row it found, and an empty list has none. `slotWrittenAt` is indexed through a `k:` prefix (`stampKey`) rather than by the bare key, so no key can reach `Object.prototype` and the record needs no `isUnsafeKey` bail-out: a slot keyed `__proto__` is protected like any other, which is what makes "every write is stamped" true without exception. The stamp is the cost of reaching a mutable row at all, not something a writer opts into, which is what makes a writer added later protected by construction; `dashboardSlice.rowWriterChokepoint.test.ts` fails the build on a reducer that takes a row out of `state.slots` itself without marking the lookup read-only, policing lookup, iteration AND index access because the pre-fix `patchSlotSourceLinks` reached its rows by iterating. Deliberately scoped to the HTTP path: a `slots` frame and the `sse*` deltas arrive on one socket and so in order, and the single writer whose DISPATCH is deliberately deferred (`touchSlotActivity`, coalesced once per frame) already refuses to move either watermark backwards for exactly that reason, so ordering is not what `sseSlots` relies on. An HTTP reply carries no such ordering. Two costs are accepted: a write landing between the dispatch and the server's serialization is counted as outranking too (the client cannot separate those instants without a server-minted stamp on the reply), and a withheld row's server-only fields wait for the next authoritative frame — bounded and self-healing, where a clobbered local write has no re-delivery path at all. A write that changed nothing (a `touchSlotActivity` both watermarks rejected, a `source_status` delta matching no link on that row) is not stamped, so a no-op cannot withhold a reply. Stamps are pruned with the rest of the per-slot state when an authoritative list the caller vouches for drops the key. YOLO state is backend-authoritative: `sseStatus` reducer syncs `approvalMode` from `status.yolo`; page load fetches `/api/status` immediately for instant sync.
