...
import { readDecisionRecord, readDecisionStrip, readModelRecord } from '../pages/chat/decisionRecord'

const SPEC = join(__dirname, '../../../docs/system-specs/modules/decisions.md')
const SECTION = "## 8. The decision strip's record and feedback"
const MODEL_SECTION = '## 9. Model routing (`model.route`)'

/** The first fenced JSON block inside *section*. */
function specFixtureUnder(section: string): Record<string, unknown> {
...
function specFixture(): Record<string, unknown> {
  return specFixtureUnder(SECTION)
}
...
describe('the model-routing fixture in the decisions spec', () => {
  const fixture = specFixtureUnder(MODEL_SECTION)
