...
describe('TurnNavigationMinimap', () => {
  ...
  it('highlights on-screen turns, hides the native scrollbar, and navigates by pointer', async () => {
    ...
    // The rail replaces the native scrollbar while mounted…
    ...
    // …and gives it back when the rail unmounts.
    view.unmount()
  ...
  it('hover opens the card immediately, leaving closes it; a 28px hit area leaves the gutter clickable-free', async () => {
    const scroller = buildScroller()
    render(<TurnNavigationMinimap items={ITEMS} scrollerRef={{ current: scroller }} onNavigate={vi.fn()} />)
    const button = await screen.findByRole('button')
    button.getBoundingClientRect = () => rect(100, 300, 8, 28)
    expect(button.className).toContain('w-7')
    // No intent delay: the first mouse move over the rail opens the card.
    fireEvent.mouseMove(button, { clientY: 200 })
    expect(screen.getByRole('tooltip')).toHaveTextContent('Second prompt')
    // Moving updates instantly.
  ...
  it('the preview card shows faint neighbour titles and a position meta row with relative time', async () => {
    ...
    expect(meta.textContent).toContain('#2 of 3')
  ...
  it('a bucketed marker\'s meta row says how many turns it covers', async () => {
    const scroller = buildScroller()
    ...
    expect(screen.getByTestId('turn-navigation-preview-meta').textContent).toContain('2 turns')
  })
  ...
  it('below the md breakpoint the CSS hides the rail, so the native scrollbar must stay', async () => {
  ...
  it('windowed rail says "loaded" in the aria label and the meta row', async () => {
    ...
    button.focus()
    fireEvent.keyDown(button, { key: 'Home' })
    expect(button.getAttribute('aria-label')).toContain('of 3 loaded')
    expect(screen.getByTestId('turn-navigation-preview-meta').textContent).toContain('#1 of 3 loaded')
  })
