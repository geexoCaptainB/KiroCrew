...
describe('MembersPage auto patrol (monitor loop status)', () => {
  ...
  it('a stopped patrol lists as a muted wake source instead of reading "nothing wakes this member"', async () => {
    ...
    expect(screen.getByTestId('member-wake-patrol-stopped')).toHaveTextContent(/^stopped$/i)
    // The row carries a bare state word, not the sentence the Auto patrol block
    // shows: one drawer saying "Patrol stopped." twice gave a reader no way to tell
    // whether the two lines were the same thing or two different ones.
