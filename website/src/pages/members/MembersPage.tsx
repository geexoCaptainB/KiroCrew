                ...
                >
                  ...
                  {patrolState === 'active' ? (
                    ...
                    <span className="text-muted shrink-0" data-testid="member-wake-patrol-stopped">
                      {/* A bare state word, not the sentence the Auto patrol block
                          above already shows: this row lists a wake SOURCE, so its
                          trailing text reads as that source's status the way a
                          disabled job's does. Repeating "Patrol stopped." here left
                          one drawer saying it twice with no way to tell whether the
                          two were the same thing. `stopped_indicator` is reused
                          rather than duplicated: it is already this namespace's word
                          for this state, so a second key would carry an identical
                          value in all 13 locales. */}
                      {t('pages.membersPage.stopped_indicator')}
                    </span>
                  )}
                </li>
