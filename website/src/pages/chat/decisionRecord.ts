 ...
 *
 * The gateway stamps the decisions of a turn on the assistant row that ends it:
 * the skill set chosen by asking Jev (`decisions/points/skills_select.py`), and
 * the model tier it routed the turn to (`decisions/points/model_route.py`). The
 * strip renders those records and nothing else: it makes no request of its own to
...
import { DECISIONS_LIVE_POINT, DECISIONS_MODEL_POINT } from '../settings/decisionsPreview'

/** Most records one reply may carry, mirroring the gateway's own per-session cap.
 *
 *  Two points ship, so this bounds a producer that stamped something unexpected
 *  rather than anything real: a row is rendered, so an unbounded list would be an
 *  unbounded number of mounted components in one transcript row.
 */
const MAX_RECORDS = 8
...
/**
 * Validate one raw record of EITHER point. `null` means "draw nothing".
 *
