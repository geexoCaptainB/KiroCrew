 ...
 *  same floor: below it the nav is display-hidden by CSS alone, and hiding the
 *  native scrollbar there would leave the transcript with NO scroll indicator. */
...
/** The rail replaces the native scrollbar at the pane's right edge, so it
 *  needs free space between the content column and that edge. */
function hasSafeRightGutter(scroller: HTMLDivElement): boolean {
  ...
  return !!contentRect && scrollerRect.left + scroller.clientWidth - contentRect.right >= MIN_GUTTER_PX
}
...
export default function TurnNavigationMinimap({
  ...
  // Buckets with a turn currently on screen — the accent highlight, and where
  ...
  const placePreview = useCallback((bucketIndex: number) => {
    ...
    // The rail sits at the pane's right edge, so the card floats to its left.
    setPreviewLeft(Math.max(VIEWPORT_EDGE_PX, rect.left - PREVIEW_GAP_PX - PREVIEW_WIDTH_PX))
  }, [])
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      frame = requestAnimationFrame(() => {
        const gutter = hasSafeRightGutter(scroller)
        setHasGutter(gutter)
        // The rail IS the position indicator: while it is up, the native
        // scrollbar is hidden (wheel/trackpad/keys still scroll). When the rail
        // cannot render — narrow pane, coarse pointer — the scrollbar returns.
        // `scrollbarWidth` is not a property TranscriptScrollShell claims, so
        // this stays inside its documented style contract.
        scroller.style.scrollbarWidth = gutter ? 'none' : ''
        if (!gutter) {
          setVisible(prev => (prev.size === 0 ? prev : new Set()))
          return
        }
  ...
  const label = selected !== null && active && activeBucket
    ? i18nT(windowed ? 'pages.chatPage.turn_minimap_active_windowed' : 'pages.chatPage.turn_minimap_active', { current: activeBucket.first + 1, count: items.length, prompt: active.label })
  ...
  const activeMeta = activeBucket
    ? activeBucket.size > 1
      ? i18nT('pages.chatPage.turn_minimap_meta_grouped', { count: activeBucket.size, first: activeBucket.first + 1 })
  ...
  const onMouseMove = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    // Immediate by design: the rail sits where the pointer deliberately aims,
    // so hover opens the card with no intent delay.
    select(pointerToTurnIndex(event.clientY, rect.top, rect.height, buckets.length))
  }
  ...
  return (
    ...
    // transcript scrolls exactly as before without it (with its native
    // scrollbar back).
    <nav
      data-testid="turn-navigation-minimap"
      aria-label={i18nT('pages.chatPage.turn_minimap_landmark')}
      className="absolute right-2 top-20 bottom-36 z-[3] hidden md:flex w-10 pointer-events-none items-center justify-end"
    >
      ...
      <button
        ...
        className="relative w-7 cursor-pointer pointer-events-auto bg-transparent border-none p-0 touch-none select-none focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg)] rounded"
      ...
      >
        {buckets.map((bucket, index) => (
          <span
            ...
            aria-hidden
            className="absolute right-0 block rounded-full"
            style={{
      ...
      {active && activeBucket && createPortal(
        // The card floats left of the rail: the scrubbed turn in its
        ...
        >
          ...
          <div data-testid="turn-navigation-preview-meta" className="mt-1.5 text-[10.5px] leading-[14px] text-muted opacity-80">
            {activeMeta}
