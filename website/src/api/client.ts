...
function showSessionExpiredBanner(lead?: string): void {
  ...
  // fragments, which left the banner untranslated in every locale while the
  // panel's own error sentence was translated. It is one catalog string now,
  // split on `{{cmd}}` so the <code> element carrying the command survives and
  // a translator can move the command to wherever the target grammar needs it.
  // One complete sentence in one key, with the command inside it.
  //
  ...
  instruction.textContent = i18nT('api.client.run_kirocrew_token_then_paste_sign_in_url')
