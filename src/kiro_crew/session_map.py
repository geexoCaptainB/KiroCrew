...
def set_bind_listener(callback: BindListener | None) -> None:
    ...
    before the announcement has been made. The file write is only QUEUED at that point,
    and deliberately so -- the ordering that matters is against readers of the map, and
    they read the dict, not the file. A sink that waited for the disk would hold the lock
    across a write without buying any ordering the in-memory commit does not already give.
