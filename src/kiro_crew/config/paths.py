...
def kiro_home() -> Path:
    ...
    * ``skills`` (``agent.py`` skill-path discovery, ``dashboard/handlers/
    ...
    if not override:
        return default_kiro_home()
    p = _explicit_kiro_home()
    if p is None:
...
def default_kiro_home() -> Path:
    """The machine-wide kiro-cli home, ``~/.kiro``, regardless of ``KIRO_HOME``.

    The one spelling of the host home, for the few callers that must name it
    while the process itself reads another: the doctor line that says what the
    host still holds, and the transcript reclaim that moves an adopted
    instance's files out of it. Everything else goes through :func:`kiro_home`.
    """
    return Path.home() / ".kiro"


_explicit_kiro_home_cache: tuple[str, Path] | None = None


def _explicit_kiro_home() -> Path | None:
    ...
    Successful resolutions are memoized on the raw environment string, so the
    hot path does not repeat filesystem I/O and a changed override naturally
    misses. A symlink retargeted under an UNCHANGED string keeps the memoized
    resolution until restart, the same freshness ``ensure_data_home()``'s memo
    already gives the data home. Invalid or temporarily unresolvable values are
    deliberately not
    cached: the same spelling may become valid after a mount or symlink repair.
    ...
    """
    global _explicit_kiro_home_cache

    override = os.environ.get("KIRO_HOME")
    if not override:
        return None
    cached = _explicit_kiro_home_cache
    ...
    if _is_unsafe_home(p):
        return None
    _explicit_kiro_home_cache = (override, p)
    return p
...
def adopt_isolated_kiro_home() -> Path | None:
    ...
    logger.info(
        ...
        "specs, since a non-default data home never writes the shared ones. Transcripts "
        "already moved into %s remain there. `kirocrew doctor` (Data Home) reports the same.",
        own_home,
        adopted,
        Path.home() / ".kiro",
        adopted,
