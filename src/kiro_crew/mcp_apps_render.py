...
def _take_claim(
    ...
    # timestamps are still recorded, for transcript order and for a reader
    # written before the ids. A ``tool_call_id`` carries no
    ...
    # The id is still recorded, for diagnostics only, so a leaked claim stays
    # traceable to the call it came from.
    #
...
def _claim_payload(
    ...
    ``rows`` is their ``ts`` and is kept for transcript order and for a reader
    that predates the ids. A ``ts`` does not name a row: ``_ChatSlot.append``
    ...
    return json.dumps(
        {
            ...
            "rows": sorted(_bounded_row_ids(list(row_ts))),
            "row_ids": sorted(_bounded_row_ids(list(row_ids))),
        }
    ).encode()
...
def _attribute_spent_claim(
    ...
    try:
        ...
        if not raw:
            ...
            logger.info(
                "mcp-apps claim for %s carries no identity; replay is inert",
                spool_id,
            )
            return "inert"
        ...
        # nothing.
        owned_ids = (
            {r for r in claimed_ids if isinstance(r, str) and r}
...
def load_claimed_row_groups(slot_key: str) -> list[set[str]]:
    ...
    for entry in entries:
        if not entry.name.endswith(".rendered"):
            continue
        try:
            owner = json.loads(Path(entry.path).read_bytes().decode())
        except (OSError, ValueError, UnicodeDecodeError):
...
def apply_claimed_rows(groups: list[set[str]], rows: list[dict[str, Any]]) -> int:
    ...
    # A ts names exactly one row, and `_run_chat` excludes every pre-turn row from
    # the claim it writes, so two claims cannot name one row and first-wins here
    # is not a choice between live candidates.
...
async def handle_tool_result(
    ...
    *,
    ...
    is attributed by; the timestamps are kept for order and for a claim written
    before the ids existed. A ``ts`` cannot identify a row on its own -- a
    ...
    recorded beside them for transcript order and for a reader written before the
    ids. Neither is a ``tool_call_id``, which is the backend's own session-local
