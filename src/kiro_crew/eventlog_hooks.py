...
def submit(fn: Callable[[], None]) -> bool:
    ...
    """
    global _drain_registered, _dropped_appends, _overflowing
    try:
        pool = io_executor()
        with _inflight_lock:
            ...
            # Checked BEFORE submit, under the same lock the set is mutated under,
            # so the decision cannot be made against a count another thread is
            # already past. Refusing after submit would bound nothing: the closure
            # is queued by then and the memory already spent.
            if len(_inflight) >= MAX_PENDING_APPENDS:
                _dropped_appends += 1
