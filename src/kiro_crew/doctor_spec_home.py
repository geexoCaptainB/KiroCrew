...
from kiro_crew.validation import MAX_SHORT_STRING, clamp_to_max_len

logger = logging.getLogger(__name__)

# Match the existing doctor section convention that shows five concrete rows,
# then collapses the remainder to a numeric summary.
DRIFT_REPORT_MAX_ITEMS = 5
DRIFT_FIELD_MAX_CHARS = MAX_SHORT_STRING
...
class HomeDriftReport:
    ...
    verified_managed: int = 0  # managed server entries whose env shape was readable
    drift: list[HomeDrift] = field(default_factory=list)
    managed_omitted: int = 0
    foreign_omitted: int = 0
...
def _bounded_drift_field(value: object) -> str:
    """Bound one untrusted field before it enters the retained doctor report."""
    text = str(value)
    if len(text) <= DRIFT_FIELD_MAX_CHARS:
        return text
    return clamp_to_max_len(text, DRIFT_FIELD_MAX_CHARS)
...
def check_spec_home_drift(*, agents_dir: Path | None = None) -> HomeDriftReport:
    ...
    for spec_path in entries:
        ...
        for server, entry in servers.items():
            ...
            managed = spec_path.name in managed_names and server_name in MANAGED_MCP_SERVER_NAMES
            pinned = _pinned_home(entry)
            if pinned is None:
                continue
            if managed:
                report.verified_managed += 1
            if not pinned or _lexical(pinned) in expected_forms:
                continue
            drift = HomeDrift(
                spec=_bounded_drift_field(spec_path.name),
                server=_bounded_drift_field(server_name),
                pinned=_bounded_drift_field(pinned),
            ...
            if len(report.drift) < DRIFT_REPORT_MAX_ITEMS:
                report.drift.append(drift)
            elif managed:
                report.managed_omitted += 1
            else:
                report.foreign_omitted += 1
    return report
...
def doctor_spec_home_drift(issues: list[str], *, agents_dir: Path | None = None) -> None:
    ...
    *issues* and doctor exits nonzero, with the one-command remedy on the line.
    ...
    if not report.drift:
        if not report.verified_managed:
            print("  pins:        ⏹ no managed MCP servers found")
            return
        print(
    ...
    if report.managed or report.managed_omitted:
        print(
    ...
    omitted = report.managed_omitted + report.foreign_omitted
    if omitted:
        print(f"  … +{omitted} more drifted MCP server(s) omitted")
