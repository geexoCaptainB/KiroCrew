...
def _untrack_session_pid(pid: int) -> bool:
    ...
    ``retire_windows_tree_tracking`` fails closed on a False.
    ...
    compaction/reset case above -- keeps its mapping. It is hygiene, not a
    control, so it never changes the returned verdict.
    """
    prefix = f"{os.getpid()}:{pid}"
    with _session_pid_file_lock():
        path = _session_pid_file_path()
        # An absent tracking file leaves nothing to rewrite, and that counts as
        # retired -- but the mapping retraction below is a different file and
        # still owed, so record the verdict instead of returning out of the
        # function.
        rewritten = True
        if path.exists():
            lines = path.read_text(encoding="utf-8").splitlines()
            ...
            rewritten = _rewrite_pid_file(path, "\n".join(lines) + "\n" if lines else "")
