...
class TestTheCarrySurvivesNonUtf8PreProjectionFiles(unittest.TestCase):
    ...
    more, by the carry into the crew log on a crew's first write; a non-UTF-8 one
    must be skipped there rather than fail the write that carries it."""
    ...
    def test_a_non_utf8_work_item_file_is_skipped_by_the_carry(self):
    ...
    def test_a_non_utf8_skip_index_is_skipped_by_the_carry(self):
        cid, sid = self._crew_with_a_log()
