...
because this runs on the read path of a crew's every cycle.
...
units STRICTLY: a listing that failed is a refusal (503), never an empty record --
a read that cannot list them answers the empty record, but a write validated
against one could admit a second editor. The synthetic answer -- the fold advanced over the entry as sent, with
...
machinery a cold fold uses. A changed unit list, a unit whose seq went backwards, or
growth in any unit but the newest forces a cold rebuild -- each would otherwise be a
...
item key by key or carry an oversized member into every retained item. `counts`
...
ONCE more, on a crew's first write after the upgrade when its fold is empty --
...
while its open items sat unread -- and
...
labels trimmed to what the fold keeps -- and a row no clamp can fit is NOT carried
and does NOT let the carry finish: the finished marker would discard stored state
for good, so the files stay unmarked, the rows are named in the log, and the next
write tries again. A file that appears later beside a crew with
a live record -- a restore from a backup -- is left alone. The files are never
