...
def _state_lock():
    ...
    first wins. :func:`_locked_state_update` and the Layer B upload gate in
    :func:`run_sessions_backup` are its two holders.
...
def _record_run(
    ...
    *,
    ...
    with _run_lock:
        return _record_run_locked(account, kind, key, size, fingerprint, version, layer_b=layer_b)
...
def run_sessions_backup(
    ...
    with tempfile.TemporaryDirectory(prefix="kc-backup-") as tmp:
        ...
        # and the run record is written after the block. `_record_run` takes the
        # same lock, so keeping it outside is what prevents a deadlock. The
        # retention sweep takes the same FILE lock under `_RETENTION_GATE`, but it
        # runs after this block has released, not inside it.
        #
        # The cost is that a same-account revocation, `_record_run` and the nightly
        # loop wait for the in-flight upload, bounded by `_PUSH_TIMEOUT_SECS`. A
        # revocation that appears slow is the price of one that cannot be
        # overtaken, and the exposure it prevents has no recovery.
        with _state_lock():
