...
def _copy_tree_without_links(
    ...
    skipped: list[str] = []
    if budget is None:
        budget = {"files": MAX_SKILL_FILES, "bytes": MAX_SKILL_TREE_BYTES}
    ...
    try:
        ...
        skipped.append(f"unreadable, skipped: {src} ({exc.strerror or exc})")
        return copied, skipped
    for entry in entries:
        target = dst / entry.name
        if is_link_or_junction(entry):
            skipped.append(f"link skipped: {entry}")
            continue
        if entry.is_dir():
        ...
        if budget["files"] <= 0:
            skipped.append(f"over {MAX_SKILL_FILES} files in this skill, skipped: {entry}")
            continue
        if size > budget["bytes"]:
            skipped.append(f"over {MAX_SKILL_TREE_BYTES} bytes in this skill, skipped: {entry}")
            continue
        try:
            ...
            skipped.append(f"unreadable, skipped: {entry} ({exc.strerror or exc})")
            continue
...
def _bounded_manifest_value(
    ...
    retained at all -- past the depth limit -- because at that point there is no
    truncated form that still means anything.
    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            ...
            if len(key) > MAX_MANIFEST_KEY_CHARS:
                ...
                continue
            if len(out) >= MAX_MANIFEST_CONTAINER_ITEMS:
                report.warnings.append(
    ...
    if isinstance(value, list):
        ...
        for index, item in enumerate(value):
            if len(items) >= MAX_MANIFEST_CONTAINER_ITEMS:
                report.warnings.append(
        ...
        return items, True
    # bool/int/float/None are fixed-size, so they carry no growth to bound.
    return value, True
