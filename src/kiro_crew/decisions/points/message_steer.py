...
def redacted(text: str, limit: int) -> str:
    ...
    The tail is taken FIRST so a long body costs a bounded redaction, and clipped
    AGAIN afterwards because a replacement token can be longer than what it
    replaced. Never raises: a redactor that fails yields nothing rather than
    unredacted text, which is the only safe direction for text about to leave the
    machine.
    """
    ...
    try:
        ...
        cleaned, _warnings = redact_credentials(tail)
...
def running_turn_excerpts(rows: Sequence[Mapping[str, Any]], budget: int) -> tuple[str, str]:
    ...
    *budget* is the consented ceiling (:func:`turn_budget`) and covers BOTH halves
    ...
    Both halves come back REDACTED and clipped, so a caller cannot forget either.
    ...
    for entry in reversed(tail):
        ...
        if role not in ACTIVITY_ROLES or spent >= activity_room:
            continue
        piece = content[: activity_room - spent]
        pieces.append(piece)
        spent += len(piece)
    pieces.reverse()
