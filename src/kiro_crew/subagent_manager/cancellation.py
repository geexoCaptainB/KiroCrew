...
class CancellationCoordinator(ManagerComponent):
    ...
    def _boundary_cancellation_pending_impl(self, params: Mapping[str, object]) -> bool:
        ...
        return bool(
            parent and owner and (parent, owner) in self._manager._pending_boundary_cancellations
        )
    ...
    def boundary_cancellation_pending_reason_impl(
        ...
        """Latest durable-settlement failure for one exact cancellation scope."""
        return self._manager._pending_boundary_cancellations.get(
    ...
    async def cancel_for_boundary_impl(
        ...
        if not parent_session_key or not boundary_owner:
            return (0, 0)
        scope = (parent_session_key, boundary_owner)
        self._manager._pending_boundary_cancellations.setdefault(scope, "")
        # The scope decision is synchronous. Revoke live work, completed reports,
        # and watcher-owned follow-ups before the durable writer can suspend.
