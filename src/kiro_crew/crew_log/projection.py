...
The panel reads five. ``class`` is a sixth registered fold with no panel: its reader is
...
#: The session side panel's projections, in the RFC section 5 order, then the
#: folds added after it. These fold ONE session's crew log and are the set the
#: growth push sends. ``class`` is last because it is not a panel value: it
#: answers what kind of session the log belongs to over the log's whole life, for
#: a reader deciding whether another session may read it. It is registered here
#: rather than kept private so that one machinery folds it -- the same checkpoint,
#: the same incremental reuse, the same recreated-log guard -- and because a fold
#: of the class reveals nothing a reader entitled to the log could not read off
...
def fold_class(entries: Iterable[Entry]) -> dict[str, Any]:
    """What kind of session this log belongs to, over its whole recorded life."""
    return fold("class", entries)
...
class SessionProjections:
    ...
    One pass over the file serves all six, which is what makes pushing the whole
...
def _advance_all(
    ...
    The per-checkpoint filter is what lets one chunk serve six folds that may sit
...
def _class_render(state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        # How many entries stated a class, opener included. A reader does not branch
        # on it; it is here so an operator can see that a fold reporting a
        # restrictive value was reading a move rather than an opening.
        "stated": state["stated"],
    }
