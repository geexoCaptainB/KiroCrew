...
def _candidate_is_same_slot(candidate_sid: str, slot: str) -> bool:
    ...
    The open is a read: ``CrewLog.open`` claims write ownership only when asked to
    repair, and nothing here asks. Called once per entry, behind the latch, so a
    retry re-reads nothing.

    Only the store's own error and an I/O error are caught, NOT every exception. A
    bare ``except`` here would turn a mistake in this function into a permanently
    silent "not the same slot", which reads exactly like a correct refusal and would
    disable the check for every slot at once. Anything other than an unreadable crew
    log is a bug and belongs in the log, not swallowed.
    """
    if not slot or not candidate_sid:
        return False
    try:
        header = _crew_log().CrewLog.open(_KIND, candidate_sid).header
    except (OSError, _crew_log().CrewLogError):
        return False
    return getattr(header, "slot", None) == slot
