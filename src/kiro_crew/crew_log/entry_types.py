...
_SESSION_TYPES: tuple[EntryType, ...] = (
    ...
    EntryType(
        ...
        note=(
            "The class as re-observed on a later turn, written only when it differs "
            "from the last one recorded. The opening entry states the class as of the "
            "moment the log was created, and a session can acquire a channel surface, "
            ...
            "Observed at the START of a turn, which is what makes turn-boundary "
            "sampling sound rather than approximate: a channel link exists before the "
            "inbound message it routes, so the turn that carries a third party's words "
            "into the log is a turn whose opening observation already saw the link that "
            "carried them. A class acquired part-way through a turn is recorded on the "
            "next one, and the only content inside that window is the session's own.\n\n"
