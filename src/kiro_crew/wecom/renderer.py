...
class WeComRenderer(Renderer):
    ...
    def _reseam(self, answer: str) -> None:
        ...
        The guard is the sealed edge's EXISTENCE, not ``_carried``. Zero is a legal
        ...
        a key across the boundary. The edge is zero only before any bubble was sealed,
        and an empty head completes nothing, so skipping there is right.
        """
        if self._frozen_end:
            self._carried = safe_resume_offset(
                answer[: self._frozen_end],
                answer,
                self._carried,
                _default_redactor,
            )
