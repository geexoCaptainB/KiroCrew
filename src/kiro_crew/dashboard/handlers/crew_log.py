...
async def api_session_crew_log_projections(request: web.Request) -> web.Response:
    ...
    It serves EVERY registered fold, not the subset a panel draws, because the cost
...
# transitive walk is ``crew_log.tree``'s own fold rather than a second slot map
...
def _recorded_class_refusal(recorded: dict[str, Any] | None) -> str:
...
def _target_class_refusal(request: web.Request, unit: str, recorded: dict[str, Any] | None) -> str:
    ...
    """
    refusal = _recorded_class_refusal(recorded)
    if refusal:
        return refusal
...
async def _read_scope_refusal(request: web.Request, session_key: str, unit: str) -> str:
    ...
    # time.
    return _target_class_refusal(request, unit, recorded)
...
def _list_scope(request: web.Request, session_key: str) -> tuple[str, str, str]:
    ...
    * a caller in one of the excluded classes gets its own unit and nothing below
    ...
    The rows a listing returns carry no class test. A row is a unit id, its slot and
    its counts, and the scope has already decided the caller may see the sessions it
    dispatched; a class test here would take the recorded class of every candidate
    and turn a listing into one log read per unit on the host.
    """
    class_refusal = _caller_class_refusal(request, session_key)
    if not class_refusal and not _owner_session_refusal(request, session_key):
        return "", "", ""
...
async def _stale_grant_refusal(
    request: web.Request, operation: str, *, unit: str = ""
) -> web.Response | None:
    """``None`` when the target's recorded class still holds, else the refusal.
    ...
    What this deliberately does NOT re-test is the CALLER's class, and the reason is
    that such a test could not buy an invariant. A caller's class says where its turn
    LANDS, and that is decided when the turn is emitted -- after this function has
    returned. A caller that acquires a channel link one instant after the payload
    returns, or that already holds the payload in context on its next turn, publishes
    the same content with nothing to stop it, and no refusal here can claw back a read
    already granted. Re-testing the caller would therefore shrink one race window by
    the duration of a read while leaving the adjacent instant open, and would read in
    the source as a guarantee the route cannot make. The caller's class is tested at
    the GATE, where it decides whether the read happens at all. That test is real; a
    second one here would not be.

    The mark under :data:`TARGET_CLASS_KEY` survives for the target half alone: it says
    whether a target test is OWED. An owner-granted read must not be target-tested,
    because an owner reads a published unit by entitlement, so without the mark this
    would refuse what the gate deliberately allowed.
    ...
    A listing is not re-checked. Its scope is a filter rather than a verdict on rows,
    and the only thing that could have moved under it is the caller's own class, which
    is the test this function does not make.
    ...
    if granted == _OWNER_GRANT:
        ...
        # distinction past the offload, which is why it survives the caller half.
        return None
    if granted != _LINEAGE_GRANT:
        ...
        refolded = await asyncio.to_thread(recorded_class, unit)
        refusal = _target_class_refusal(request, unit, refolded)
    if not refusal:
        return None
    from kiro_crew.dashboard.token_auth import request_origin
...
async def api_crew_log_sessions(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return _crew_log_refusal(exc)
    # No re-check here. A listing's scope is a FILTER rather than a verdict on rows,
    # so the only thing that could have moved under it during the read is the caller's
    # own class -- and a caller-class re-test cannot buy an invariant, since where a
    # turn lands is decided after this route returns. See _stale_grant_refusal.
    return web.json_response(payload)
...
class CrewLogPublisher:
    ...
    rather than a read of the whole file, which is what makes pushing all six
