...
from kiro_crew.skills import _trusted_skill_roots, _within_any, skills_dir
...
def _leaf_is_contained(resolved: Path, root_resolved: Path, prefix_allows_link: bool) -> bool:
    ...
    * an app skill provider — ``apps.bridges._register_skills`` symlinks each
      app skill into the kirocrew skills root (both ``skills/<app>/<skill>`` and
      a flat ``skills/<skill>``) with the target in the app's own tree, so those
      resolved paths land outside the root by construction.

    The provider roots come from ``skills._trusted_skill_roots``, the same
    helper the loader-side walk admits them with. Two lists would drift, and a
    browse side stricter than the loader shows skills in ``GET /api/skills``
    ...
    if prefix_allows_link:
        return True
    return _within_any(str(resolved), _trusted_skill_roots())
