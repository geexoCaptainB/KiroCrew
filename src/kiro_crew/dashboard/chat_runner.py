...
async def _run_chat(
    ...
    *,
    ...
    try:
        ...
        #
        # `mapped_sid` is the FALLBACK, not the first choice, and only `mapped_sid`
        # of the two mapping accessors. `resumable_sid` asks "can this id still be
        ...
        # resume would not.
        #
        # The emitter is asked FIRST because the mapping can be behind. An
        # allocation whose replay is still pending defers publishing its fresh id,
        # so the mapping keeps naming the store before it; two successive
        # allocations then cite that same older store and the store between them is
        # cited by nobody, which a walker steps over with no signal. The emitter
        # records what it actually opened, so it names the newest store whether or
        # not its id was ever promoted. It answers nothing on a cold start, and
        # there the mapping is the only record that outlived the process.
        #
        # Read non-pruning, and `mapped_sid` rather than the resumable accessor for
        # the reason given at its definition: this asks what the key was last
        # serving, not what can still be resumed. The replay-pending window, where
        # the mapping names the store before the newest one, is a recorded residual
