    ...
    *keep* holds resources that must live until the arrival is finished rather
    ...
    try:
        ...
        try:
            ...
            await discard_arrival_folders(state, created_folders)
            logger.warning(
    ...
    async def _refuse_as_deleted(witness: str) -> web.Response:
        ...
        if current is slot:
            ...
            # landed and was itself popped, the mapping at ``sm_key`` could be
            # that replacement's, and unlinking the sid it names would delete its
            # files -- the precise harm the object comparison exists to prevent.
            # The locally known sid cannot reach another session's files, so the
            # residual is a foreign MAPPING possibly forgotten, never foreign
            # files, and reaching it needs three events inside this one tail where
            # the case above needs one.
    ...
    # ONE WITNESS ON EVERY PATH TO SUCCESS. The guard above only fires when the
    # arrival FOLDER went away, so on its own it leaves the common case unchecked:
    # a ``DELETE /api/sessions/{key}`` landing in the folder-existence await
    # removes this transcript and pops the slot while the folder it pointed at is
    # still perfectly fine, so the branch is skipped and the handler would answer
    # ``200 ok`` for data that has already been destroyed. Nothing downstream
    # corrects that -- the success return does not re-arm ``_dirty`` and the
    # delete's pop is terminal -- so the misleading ``ok`` is the permanent record.
    #
